...
PTERM *t;
	...
	/**********************************************************/
	/*** First, initialize the 3 notorious splmem locations ***/
	/**********************************************************/
	addtag(&(splmem[SPLMEM_SEE_INPUT_T]), TAG_CON, USER); /* NEWTAG see_input_t */
	addtag(&(splmem[SPLMEM_EFILE_1]), TAG_STR, QUERY); /* NEWTAG efile[0] */
	addtag(&(splmem[SPLMEM_EFILE_2]), TAG_CON, END); /* NEWTAG efile[1] */
...
*/
	...
	/* NEWTAG infile_t = addtag(TAG_CON, STDINVAL); */ addtag(&(splmem[SPLMEM_INFILE_T]), TAG_CON, STDINVAL);
