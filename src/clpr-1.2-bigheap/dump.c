...
int TAG_COLLECT_DUMP_VARS = 0;
int BOOL_COLLECT_DUMP_VARS = FALSE;
...
int *tmp;
	...
	/* NEWTAG *first_constraint_list = addtag(TAG_NIL, EMPTYLIST); */ addtag(first_constraint_list, TAG_NIL, EMPTYLIST);
	init_dump();
#ifdef BIGHEAP
	splmem[SPLMEM_NEW_PROJ_VARS] = collect_vars(vars, NULL, 3);
	if(BOOL_COLLECT_DUMP_VARS) {
		addtag(&splmem[SPLMEM_NEW_PROJ_VARS], TAG_COLLECT_DUMP_VARS, splmem[SPLMEM_NEW_PROJ_VARS]);
		BOOL_COLLECT_DUMP_VARS = FALSE; TAG_COLLECT_DUMP_VARS = 0;
	}
#else
	...
	add_flatten_eqns();
	/* NEWTAG *constraint_list = addtag(TAG_NIL, EMPTYLIST); */ addtag(constraint_list, TAG_NIL, EMPTYLIST);
#ifdef BIGHEAP
	splmem[SPLMEM_TMP] = first_constraint_list;
#else
...
#ifdef BIGHEAP
	if (!unify(&(splmem[SPLMEM_TMP]), into)) return FALSE;
#else
...
**/
#ifdef BIGHEAP
	if (!unify(&(splmem[SPLMEM_NEW_PROJ_VARS]), user_vars)) return FALSE; 
#else
...
char *namep, *varname;
	namep = names;
#ifdef BIGHEAP
	TAG_COLLECT_DUMP_VARS = 0; BOOL_COLLECT_DUMP_VARS = FALSE;
	splmem[SPLMEM_FIRST] = (INTptr) NULL;
	prev_list = (INTptr) &(splmem[SPLMEM_FIRST]);
#else
	first = (INTptr) NULL;
	...
	for (listp = findbind(vars); tag(*listp) != TAG_NIL; 
		...
		if (tag(*listp) == TAG_CONS) {
			if (!is_dump_output) {
				/* NEWTAG *prev_list = addtag(TAG_CONS, heaptop); */ addtag(prev_list, TAG_CONS, heaptop);
				TAG_COLLECT_DUMP_VARS = TAG_CONS; BOOL_COLLECT_DUMP_VARS = TRUE;
	...
	if (!is_dump_output) {
		/* NEWTAG *prev_list = addtag(TAG_NIL, EMPTYLIST); */ addtag(prev_list, TAG_NIL, EMPTYLIST);
		if(!v_test) { TAG_COLLECT_DUMP_VARS = TAG_NIL; BOOL_COLLECT_DUMP_VARS = TRUE; }
	}
#ifdef BIGHEAP
	return splmem[SPLMEM_FIRST];
#else
	return first;
#endif
}
...
int *t;
	t = heaptop;
	/* NEWTAG *(heaptop++) = addtag(TAG_STR, val); */ addtag((heaptop++), TAG_STR, val);
...
int *t;
        t = heaptop;
        /* NEWTAG *(heaptop++) = addtag(TAG_STR,HAT_U_MINUSVAL); */ addtag((heaptop++), TAG_STR,HAT_U_MINUSVAL);
        make_bound_var(heaptop++,t1);
        return t;
}
...
int *e;
{
	if (!e) return;
	if (dump_mode) {
		/* NEWTAG *constraint_list = addtag(TAG_CONS, heaptop); */ addtag(constraint_list, TAG_CONS, heaptop);
		make_bound_var(heaptop, e);
		constraint_list = heaptop + 1;
		heaptop += 2;
	} else {
		/* WHAT A SUBTLE BUG!!! NEWTAG *constraint_list = (int) heaptop; */ make_bound_var(constraint_list, (int) heaptop);
		/* NEWTAG make_bound_var(heaptop, addtag(TAG_STR, COMMA)) */ make_bound_var_val(heaptop, TAG_STR, COMMA); 
...
int *t;
	...
	/* NEWTAG *heaptop = addtag(TAG_MISC, NULL); */ addtag(heaptop, TAG_MISC, NULL);
	t = heaptop++;
	return t;
}
...
int i;
	for (i = 1; i <= num_vec_top; i++) {
		...
		/* NEWTAG *(num_vec[i].loc) = addtag(TAG_PAR, solver_id); */ addtag((num_vec[i].loc), TAG_PAR, solver_id);
	}
}
...
int *t;
	t = heaptop;
	/* NEWTAG *(heaptop++) = addtag(TAG_STR, EQVAL); */ addtag((heaptop++), TAG_STR, EQVAL);
	make_bound_var(heaptop++, t1);
	make_bound_var(heaptop++, t2);
	return t;
}
...
int *t;
	t = heaptop;
	heaptop = heaptop + 3;
	/* NEWTAG *t = addtag(TAG_CONS, t + 1); */ addtag(t, TAG_CONS, t + 1);
...
int i, *t, *t1;
	t = heaptop;
	heaptop = heaptop + n + 1;
	/* POSSIBLE BUG. REMOVE BIGHEAP IN THAT CASE */ 
#ifdef BIGHEAP
	make_bound_var_val(t, tag(*t0), *t0);
#else
	*t = *t0;
#endif
