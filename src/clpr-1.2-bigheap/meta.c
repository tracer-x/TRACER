...
int *t, *t3, *t4, *base, n, i;
	...
	case TAG_CONS:
		base = heaptop; 
		heaptop = heaptop + 3;
		/* NEWTAG *base = addtag(TAG_CONS, base + 1); */ addtag(base, TAG_CONS, base + 1);
...
int *tmp;
	...
	} else /* NEWTAG make_bound_var(tmp, *t); */ make_bound_var_val(tmp, tag(*t), *t);
	return tmp;
}
...
int *base;
	base = heaptop;
	heaptop = heaptop + 2;
	/* NEWTAG *base = addtag(TAG_MISC, EVALVAL); */ addtag(base, TAG_MISC, EVALVAL);
	/* NEWTAG make_bound_var(base + 1, addtag(TAG_MISC, solve_eval(t1, t2))) */ make_bound_var_val(base + 1, TAG_MISC, solve_eval(t1, t2)); 
	return base;
}
...
NL_EQN_ptr nl_eqn;
	have_runable = FALSE;
	r = runable;
	/* NEWTAG runable = (int *) addtag(TAG_NIL, EMPTYLIST); */ addtag(&runable, TAG_NIL, EMPTYLIST);
...
int *t;
{
	if (is_unbound_var(*t)) { 
		...
		/* NEWTAG make_bound_var(t, addtag(TAG_PAR, ++solver_id)) */ make_bound_var_val(t, TAG_PAR, ++solver_id); 
