...
PTERM *it;
	...
	for (j = 1, it = t->first; it; j++, it = it->next) {
		...
		if ((fmask = can_gen_uni2var(it)) && 
			...
			if (bit1(fmask)) {
				count++;
#ifdef BIGHEAP
				make_bound_var(&(trail[trtop++]), (int *) it);
				make_bound_var(&(trail[trtop++]), (int *) r1);
#else
			...
			if (bit2(fmask)) {
				count++;
#ifdef BIGHEAP
				make_bound_var(&(trail[trtop++]), (int *) it->next);
				make_bound_var(&(trail[trtop++]), (int *) r2);
#else
				store_trail_bits0(trtop); 
		...
		if ((is_arith_term(it) && !is_eval(it))
			...
			tmp = getreg();
#ifdef BIGHEAP
			make_bound_var(&(trail[trtop++]), (int *) tmp);
#else
			store_trail_bits0(trtop); 
			trail[trtop++] = (int *) tmp;
#endif
	...
	for (; count; count--) {
#ifdef BIGHEAP
		it = (PTERM *) ((unsigned)trail[tmp_trtop++]);
		tmp = (int) ((unsigned)trail[tmp_trtop++]);
#else
...
#ifdef BIGHEAP
			make_bound_var(&(trail[trtop++]), (int *) put_func(it));
#else
			store_trail_bits0(trtop);
...
int save_trtop;
	...
	for (i = 1, it = t->first; it; i++, it = it->next) {
		if (is_complex_func(it) || is_eval(it)) { 
			if (i < arity || !is_rr(it)) {
#ifdef BIGHEAP
				emit_bldtval(tmp = (int) ((unsigned) trail[trtop++]));
#else
