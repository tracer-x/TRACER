...
/*
NEWTAG int efile[2] = { addtag(TAG_STR, QUERY), addtag(TAG_CON, END) };
NEWTAG int see_input_t = addtag(TAG_CON, USER);
*/
extern int splmem[];
...
/* added by Vijay - for dealing with return addtag(...) */
int TAG_HEAPIFY_PTERM;
...
/*****/
	...
	/* Utilities added for constraint tracer project */
	{ do_lexlt, 2, "lexlt", 0},
	...
	/* Printf that always prints to stdout, added by Andrew */
...
*/
	...
	if (feof(see_input)) /* NEWTAG make_bound_var(&nt, efile); */ make_bound_var_val(&splmem[SPLMEM_NT], tag(splmem[SPLMEM_EFILE_1]), splmem[SPLMEM_EFILE_1]);
	else {
		/* NEWTAG new_input(see_input, string_of(mask(see_input_t))); */ new_input(see_input, string_of(mask(splmem[SPLMEM_SEE_INPUT_T])));
		/**
...
#ifdef BIGHEAP
			splmem[SPLMEM_NT] = (int) pterm_to_heap(exp_tt);
			addtag(&splmem[SPLMEM_NT], TAG_HEAPIFY_PTERM, splmem[SPLMEM_NT]);
#else
			nt = (int) pterm_to_heap(exp_tt);
#endif
#ifdef  FREE_PTERMS_READ
/* Added by Jorge */			
			free_pterm(tt);
			free_pterm(exp_tt);
#else			
			...
			if (is_p_real(tt) || is_p_constant(tt)) free_pterm(tt);
			if (is_p_real(exp_tt) || is_p_constant(exp_tt)) free_pterm(exp_tt);
		} else /* NEWTAG make_bound_var(&nt, (int) efile); */ make_bound_var_val(&splmem[SPLMEM_NT], tag(splmem[SPLMEM_EFILE_1]), (int) splmem[SPLMEM_EFILE_1]);
		new_input(old_cur_stream, "");
	}
#ifdef BIGHEAP
	make_bound_var_val(heaptop, tag(splmem[SPLMEM_NT]), splmem[SPLMEM_NT]);
#else
	*(heaptop) = nt;
#endif
...
FILE *f;
	...
	/* NEWTAG see_stack[see_stack_top++].t = see_input_t; */ see_stack[see_stack_top++].t = splmem[SPLMEM_SEE_INPUT_T];
...
*/
		see_input = f;
#ifdef BIGHEAP
		make_bound_var_val(&(splmem[SPLMEM_SEE_INPUT_T]), tag(*t), *t);
#else
		see_input_t = *t;
#endif
		return TRUE;
		...
		see_input = stdin;
		/* NEWTAG see_input_t = addtag(TAG_CON, USER); */ addtag(&(splmem[SPLMEM_SEE_INPUT_T]), TAG_CON, USER);
...
{
#ifdef BIGHEAP
	make_bound_var_val(heaptop, tag(splmem[SPLMEM_SEE_INPUT_T]), splmem[SPLMEM_SEE_INPUT_T]);
#else
	*heaptop = see_input_t;
#endif
	return unify(heaptop++, t);
...
int *t;
{
	if (see_input != stdin) {
		...
		/* NEWTAG files_open = file_close(functors[mask(see_input_t)].pname, files_open); */ files_open = file_close(functors[mask(splmem[SPLMEM_SEE_INPUT_T])].pname, files_open);
	...
	/* NEWTAG see_input_t = see_stack[see_stack_top].t; */ splmem[SPLMEM_SEE_INPUT_T] = see_stack[see_stack_top].t;
	return TRUE;
}
...
FILE *f;
	...
	/* NEWTAG tell_stack[tell_stack_top++].t = outfile_t; */ tell_stack[tell_stack_top++].t = splmem[SPLMEM_OUTFILE_T];
	if (strcmp(functors[mask(*t)].pname, "user")) {
	   if (!(f = file_ready(functors[mask(*t)].pname, IO_WRITE))) 
			return FALSE;
		outfile = f;
#ifdef BIGHEAP
		make_bound_var_val(&(splmem[SPLMEM_OUTFILE_T]), tag(*t), *t);
#else
		outfile_t = *t;
#endif
		return TRUE;
		...
		outfile = stdout;
		/* NEWTAG outfile_t = addtag(TAG_CON, USER); */ addtag(&(splmem[SPLMEM_OUTFILE_T]), TAG_CON, USER);
		return !strcmp(functors[mask(*t)].pname, "user");
	}
}
/* Begin added by Jorge */
...
{
#ifdef BIGHEAP
	make_bound_var_val(heaptop, tag(splmem[SPLMEM_OUTFILE_T]), splmem[SPLMEM_OUTFILE_T]);
#else
	*heaptop = outfile_t;
#endif
	return unify(heaptop++, t);
...
do_told()
{
	...
	tell_stack_top--;
	if (outfile != stdout)
		/* NEWTAG files_open = file_close(functors[mask(outfile_t)].pname, files_open); */ files_open = file_close(functors[mask(splmem[SPLMEM_OUTFILE_T])].pname, files_open);
	outfile = tell_stack[tell_stack_top].f;
	/* NEWTAG outfile_t = tell_stack[tell_stack_top].t; */ splmem[SPLMEM_OUTFILE_T] = tell_stack[tell_stack_top].t;
	return TRUE;
}
...
PTERM *arg;
	...
	case REAL:
		...
		TAG_HEAPIFY_PTERM = TAG_PAR;
		return solver_id;
	case VAR:
		if (!hp) fatal("81811");
		if (t->val >= MAX_VAR_IN_HEAP) fatal("too many variables in term");
		if (var_heapval[t->val]) {
			TAG_HEAPIFY_PTERM = 0;
			return var_heapval[t->val];
		}
		else {
	...
	case FUNC:
		...
		if (t->val == DOT) {
			...
			*s = heapify_pterm(t->first, s, quoted);
			addtag(s, TAG_HEAPIFY_PTERM, *s);
		 	TAG_HEAPIFY_PTERM = 0;
			s++;
			*s = heapify_pterm(t->first->next, s, quoted);
			addtag(s, TAG_HEAPIFY_PTERM, *s);
			TAG_HEAPIFY_PTERM = TAG_CONS;
			return (int) start;
		}
		if (!t->narg) {
			TAG_HEAPIFY_PTERM = TAG_CON;
			return t->val;
		}
		if (is_arith_term(t)) {
			...
			if (quoted) {
				t1 = heaptop;
				heaptop += 2;
				/* NEWTAG make_bound_var(t1, addtag(TAG_STR, HAT_QUOTEVAL)) */ make_bound_var_val(t1, TAG_STR, HAT_QUOTEVAL); 
				make_bound_var(t1 + 1, heapify_pterm(t, 0, FALSE));	
				addtag(t1 + 1, TAG_HEAPIFY_PTERM, *(t1 + 1));
				TAG_HEAPIFY_PTERM = 0;
				return (int) t1;
		...
		start = s = heaptop;
		heaptop += t->narg + 1;
		/* NEWTAG *s++ = addtag(TAG_STR, heap_func_val); */ addtag(s, TAG_STR, heap_func_val); s++;
...
double num;
...
#ifdef BIGHEAP
			make_bound_var(heaptop + 1, (int) findbind(head_of(tt)));
#else
			*(heaptop + 1) = (int) findbind(head_of(tt));
#endif
			tt = findbind(tail_of(tt));
#ifdef BIGHEAP
			make_bound_var(heaptop + 2, (int) findbind(head_of(tt)));
#else
			*(heaptop + 2) = (int) findbind(head_of(tt));
#endif
			heaptop += 3;
		} else {
			/* NEWTAG *heaptop = addtag(TAG_STR, hashbuiltina(functors[mask(*findbind(pmask(*l)))].pname, n)); */ addtag(heaptop, TAG_STR, hashbuiltina(functors[mask(*findbind(pmask(*l)))].pname, n));
	...
	if (n == 0) {
		/* NEWTAG *(tail_of(new_list)) = addtag(TAG_NIL, EMPTYLIST); */ addtag((tail_of(new_list)), TAG_NIL, EMPTYLIST);
		...
		/* NEWTAG *principle = addtag(TAG_CON, hashbuiltina(".", 0)); */ addtag(principle, TAG_CON, hashbuiltina(".", 0));

		/******* call to make_cons in dump.c *******/

		int *vtmp_t;
		vtmp_t = heaptop;
		heaptop = heaptop + 3;
		/* NEWTAG *t = addtag(TAG_CONS, t + 1); */ addtag(vtmp_t, TAG_CONS, vtmp_t + 1);
		make_bound_var(vtmp_t + 1, tail_of(t1));
		/* NEWTAG make_bound_var(vtmp_t + 2, addtag(TAG_NIL, EMPTYLIST)) */ make_bound_var_val(vtmp_t + 2, TAG_NIL, EMPTYLIST); 

		/******* end call to make_cons *******/
		...
		/* NEWTAG *principle = addtag(TAG_CON, hashbuiltina(string_of(mask(*t1)), 0)); */ addtag(principle ,TAG_CON, hashbuiltina(string_of(mask(*t1)), 0));
		/* NEWTAG *(tail_of(new_list)) = addtag(TAG_CONS, func_univ(t1, n)); */ addtag((tail_of(new_list)), TAG_CONS, func_univ(t1, n));
	}
...
#ifdef BIGHEAP
		make_bound_var(l, (int) (t + i));
#else
		*l = (int) (t + i);
#endif
		/* NEWTAG *(l + 1) = addtag(TAG_CONS, heaptop); */ addtag((l + 1), TAG_CONS, heaptop);
	}
	/* NEWTAG *(l - 1) = addtag(TAG_NIL, EMPTYLIST); */ addtag((l - 1), TAG_NIL, EMPTYLIST);
	heaptop = l;
	return start;
}
...
int *t, **vars;
{
	...
	t = findbind(t);
	new = heaptop;
	/* NEWTAG make_bound_var(heaptop, *t); */ make_bound_var_val(heaptop, tag(*t), *t);
	n = arity_of(mask(*t));
	for (i = 1; i <= n; i++)
...
double tagtrail_sz, misc_sz;
	...
	p_stack_sz = (unsigned) stacktop - (unsigned) STACK_START_ADDR;
	p_heap_sz = (unsigned) heaptop - (unsigned) HEAP_START_ADDR;
	p_trail_sz = (unsigned) &(trail[trtop]) - (unsigned) trail;
	p_solver_sz = (sms_blocks * SOLVER_MALLOC_NODES * sizeof(SOLVER_NODE)) + (SOLVER_SZ * sizeof(int));
	p_pterms = num_pterms * sizeof(PTERM);

	// other memory usage
#ifdef BIGHEAP
	tagtrail_sz = 0;
#else
	tagtrail_sz = (unsigned) &(tagtrail[trtop]) - (unsigned) tagtrail;
#endif

	misc_sz = HASH_P_NODE_MEMORY + EMALLOC_MEMORY;

	p_total_sz = p_code_sz + p_stack_sz + p_heap_sz + p_trail_sz + p_solver_sz
					+ tagtrail_sz + misc_sz;
...
{
	if(!head) {
		warn_err("no codespace marker available! check the number of calls to set_cs_mark/0 wrt pop_cs_mark/0.\n");
		return FALSE;
	}
	if(head->val > epcode) {
