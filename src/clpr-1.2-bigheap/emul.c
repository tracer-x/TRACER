...
int splmem[SPLMEM_SZ];	/* special memory */
char spltags[SPLMEM_SZ];
...
#ifdef BIGHEAP
int *unifystack[MAX_UNIFY_LEVELS];
char unifytags[MAX_UNIFY_LEVELS];
#endif
...
#ifdef BIGHEAP
	printf("Unifymem start: %p end: %p\n", (unsigned) unifystack, (unsigned) unifystack +  MAX_UNIFY_LEVELS * sizeof(int));
	printf("Splmem start: %p end: %p\n", (unsigned) splmem, (unsigned) splmem + SPLMEM_SZ * sizeof(int));
#endif
...
#ifdef BIGHEAP
	MEMTAGS_SZ = MSTACK + TRAIL_SZ;
	memtags = (char *) alloc_mem(MEMTAGS_SZ / sizeof(int) * sizeof(char));
#else
...
PTERM *t;
	...
	/* NEWTAG true_atom = addtag(TAG_CON, TRUEVAL); */ addtag(&(splmem[SPLMEM_TRUE_ATOM]), TAG_CON, TRUEVAL);
}
...
int *prevpc;
	...
	runable = heaptop++;
	/* NEWTAG *runable = addtag(TAG_NIL, EMPTYLIST); */ addtag(runable, TAG_NIL, EMPTYLIST);
	pc = ptrpc(ipc);
	Regs = regs;
	Regs1 = regs + 1;
	Regs2 = regs + 2;
...
/***************/
	...
	for (ti1 = trtop - 1; ti1 >= ti; ti1--) {
		ti2 = (int) trail[ti1];
		/* NEWTAG if (tag(ti2) == BTK_BIND) { */ if (tag(trail[ti1]) == BTK_BIND) {
...
#ifdef CYCLIC_BIND
		/* NEWTAG else if (tag(ti2) == BTK_TEST) */ else if (tag((int) trail[ti1]) == BTK_TEST)
#ifdef BIGHEAP
		...
		/* NEWTAG else if (tag(ti2) == BTK_TEST2) */ else if (tag((int) trail[ti1]) == BTK_TEST2)
#ifdef BIGHEAP
...
#endif
#ifdef BIGHEAP
		else solver_backtrack(ti1, &(trail[ti1]));
#else
	...
	while (tmp2 > Regs) {
#ifdef BIGHEAP
		make_bound_var_val(tmp2, tag(*tmp1), *tmp1);
		tmp2--;
		tmp1--;
#else
		*tmp2-- = *tmp1--;
...
/***************/
...
#ifdef BIGHEAP
			make_bound_var(&(Regs[1]), (int) pmask(*tmp1));
			make_bound_var(&(Regs[2]), (((int *) pmask(*tmp1)) + 1));
#else
	...
	caseop(UNILIST):
		if (mode_read) {
				...
				if (tag(*tmp1) == TAG_CONS) {
					...
					/* NEWTAG *tmp1 = addtag(TAG_CONS, heaptop); */ addtag(tmp1, TAG_CONS, heaptop);
					s = heaptop;
					mode_read = FALSE;
					...
					/* NEWTAG *heaptop = addtag(TAG_CONS, heaptop + 1); */ addtag(heaptop, TAG_CONS, heaptop + 1);
					heaptop++;
					s = heaptop;
					mode_read = FALSE;
				...
				heaptop++;
				/* NEWTAG *s = addtag(TAG_CONS, heaptop); */ addtag(s, TAG_CONS, heaptop);
				s = heaptop;
				pc++;
				goto LOOP;
		}
	caseop(UNISTR):
		if (mode_read) {
				tmp1 = s;
				deref(tmp1);
				if (/* NEWTAG *tmp1 == addtag1(TAG_STR, *(pc + 1))); */ addtagcmp(tmp1, TAG_STR, *(pc + 1))) {
					s = tmp1 + 1;
					pc += 2;
					goto LOOP;
				} else if (is_unbound_var(*tmp1)) {
					make_bound_var(tmp1, (int) heaptop);
					/* NEWTAG *heaptop = addtag1(TAG_STR, *(pc + 1)); */ addtag1(heaptop, TAG_STR, *(pc + 1));
					s = ++heaptop;
					mode_read = FALSE;
					push_check_trail(BTK_BIND, tmp1);
					pc += 2;
				...
				/* NEWTAG *(s + 1)  = addtag1(TAG_STR, *(pc + 1)); */ addtag1((s + 1) , TAG_STR, *(pc + 1));
				s += 2;
				heaptop = s;
				pc += 2;
				goto LOOP;
	...
	caseop(ENTERSWTYPE):
		...
		if (tag(*tmp1) == TAG_CONS) {
				...
				mode_read = TRUE;
#ifdef BIGHEAP
				pc = (int *) *(pc + swt_offsets[tagnum(tag(*tmp1))]);
#else
	...
	caseop(SWITCHONCON):
		...
		if (is_unbound_var(ti) || (tag(*tmp1) == TAG_DAEMON)) {
				pc = tmp2 + 2;
				goto LOOP;
		}
		pc = (int *) *(tmp2 + 1);
	...
	caseop(SWITCH2ONREAL):
		...
		if (tag(*tmp1) == TAG_PAR) {
				...
				pc = tmp2 + 2;
				goto LOOP;
		} else if (is_unbound_var(ti) || (tag(*tmp1) == TAG_DAEMON)) {
				pc = tmp2 + 2;
				goto LOOP;
		} else {
				pc = (int *) *(tmp2 + 1);
	...
	caseop(GETPVAR):
		tmp1 = curar + *(pc + 1);
#ifdef BIGHEAP
		make_bound_var_val(tmp1, tag(Regs[*(pc + 2)]), Regs[*(pc + 2)]);
#else
		*tmp1 = Regs[*(pc + 2)];
#endif
		pc += 3;
	...
	caseop(GETTVAR):
#ifdef BIGHEAP
		make_bound_var_val(&(Regs[*(pc + 1)]), tag(Regs[*(pc + 2)]), Regs[*(pc + 2)]);
#else
		Regs[*(pc + 1)] = Regs[*(pc + 2)];
#endif
		pc += 3;
	...
	caseop(GETCON):
		...
		if (tag(*tmp1) == TAG_CON) {
			...
			/* NEWTAG *tmp1 = addtag(TAG_CON, ti); */ addtag(tmp1, TAG_CON, ti);
			push_check_trail(BTK_BIND, tmp1);
			pc += 3;
			goto LOOP;
		} else if (is_daemon(tmp1)) {
			make_bound_var(tmp1, heaptop);
			/* NEWTAG *heaptop = addtag(TAG_CON, ti); */ addtag(heaptop, TAG_CON, ti);
			heaptop++;
			push_check_trail(BTK_DBIND, tmp1);
			make_runable(tmp1 + 1);
			pc += 3;
	...
	caseop(GETNIL):
		...
		if (tag(*tmp1) == TAG_NIL) {
			...
			/* NEWTAG *tmp1 = addtag(TAG_NIL, EMPTYLIST); */ addtag(tmp1, TAG_NIL, EMPTYLIST);
			push_check_trail(BTK_BIND, tmp1);
			pc += 2;
			goto LOOP;
		} else if (is_daemon(tmp1)) {
			make_bound_var(tmp1, heaptop);
			/* NEWTAG *heaptop = addtag(TAG_NIL, EMPTYLIST); */ addtag(heaptop, TAG_NIL, EMPTYLIST);
			heaptop++;
			push_check_trail(BTK_DBIND, tmp1);
			make_runable(tmp1 + 1);
			pc += 2;
...
#ifdef BIGHEAP
		make_bound_var_val(curar + 1, tag(Regs[*(pc + 1)]), Regs[*(pc + 1)]);
#else
		*(curar + 1) = Regs[*(pc + 1)];
#endif
		pc += 2;
	...
	caseop(GET1TVAR):
#ifdef BIGHEAP
		make_bound_var_val(Regs1, tag(Regs[*(pc + 1)]), Regs[*(pc + 1)]);
#else
		*Regs1 = Regs[*(pc + 1)];
#endif
		pc += 2;
	...
	caseop(GET1CON):
		...
		if (tag(*tmp1) == TAG_CON) {
				...
				/* NEWTAG *tmp1 = addtag(TAG_CON, *(pc + 1)); */ addtag(tmp1, TAG_CON, *(pc + 1));
				push_check_trail(BTK_BIND, tmp1);
				pc += 2;
				goto LOOP;
		} else if (is_daemon(tmp1)) {
				make_bound_var(tmp1, heaptop);
				/* NEWTAG *heaptop = addtag(TAG_CON, *(pc + 1)); */ addtag(heaptop, TAG_CON, *(pc + 1));
				heaptop++;
				push_check_trail(BTK_DBIND, tmp1);
				make_runable(tmp1 + 1);
				pc += 2;
...
#ifdef BIGHEAP
		make_bound_var_val(curar + 2, tag(Regs[*(pc + 1)]), Regs[*(pc + 1)]);
#else
		*(curar + 2) = Regs[*(pc + 1)];
#endif
		pc += 2;
	...
	caseop(GET2TVAR):
#ifdef BIGHEAP
		make_bound_var_val(Regs2, tag(Regs[*(pc + 1)]), Regs[*(pc + 1)]);
#else
		*Regs2 = Regs[*(pc + 1)];
#endif
		pc += 2;
...
#ifdef BIGHEAP
		make_bound_var_val(curar + 3, tag(Regs[*(pc + 1)]), Regs[*(pc + 1)]);
#else
		*(curar + 3) = Regs[*(pc + 1)];
#endif
		pc += 2;
	...
	caseop(GET3TVAR):
#ifdef BIGHEAP
		make_bound_var_val(Regs3, tag(Regs[*(pc + 1)]), Regs[*(pc + 1)]);
#else
		*Regs3 = Regs[*(pc + 1)];
#endif
		pc += 2;
...
#ifdef BIGHEAP
					make_bound_var_val(s, tag(*tmp1), *tmp1);
#else
					*s = *tmp1;
#endif
				}
...
#ifdef BIGHEAP
					make_bound_var_val(tmp1, tag(*tmp2), *tmp2);
#else
					*tmp1 = *tmp2;
#endif
				}
	...
	caseop(UNICON):
		if (!mode_read) {
				/* NEWTAG *s = addtag1(TAG_CON, *(pc + 1)); */ addtag1(s, TAG_CON, *(pc + 1));
				s = ++heaptop;
				pc += 2;
				goto LOOP;
		} else {
				...
				if (is_unbound_var(*tmp1)) {
					...
					push_check_trail(BTK_DBIND, tmp1);
								    make_runable(tmp1 + 1);
				} else if (/* NEWTAG *tmp1 != addtag1(TAG_CON, *(pc + 1))); */ !addtagcmp(tmp1, TAG_CON, *(pc + 1))) goto FAIL;
				s++;
				pc += 2;
				goto LOOP;
		}
	caseop(UNINIL):
		if (!mode_read) {
				/* NEWTAG *s = addtag(TAG_NIL, EMPTYLIST); */ addtag(s, TAG_NIL, EMPTYLIST);
				s = ++heaptop;
				pc = pc + 1;
				goto LOOP;
		} else {
				...
				if (is_unbound_var(*tmp1)) {
					...
					push_check_trail(BTK_DBIND, tmp1);
					make_runable(tmp1 + 1);
				} else if (/* NEWTAG *tmp1 != addtag(TAG_NIL, EMPTYLIST) */ !addtagcmp(tmp1, TAG_NIL, EMPTYLIST)) goto FAIL;
					s++;
					pc++;
					goto LOOP;
		}
	...
	caseop(PUTPVAL):
		...
		if (is_unbound_var(*tmp1) || is_daemon(tmp1)) {
				...
				/* NEWTAG make_bound_var(Regs + *(pc + 2), *tmp1); */ make_bound_var_val(Regs + *(pc + 2), tag(*tmp1), *tmp1);
				pc += 3;
				goto LOOP;
		}
	caseop(PUTPVALN):
		...
		do {
			...
			if (is_unbound_var(*tmp1) || is_daemon(tmp1)) 
				make_bound_var(Regs + tmp2[1], tmp1);
			else /* NEWTAG make_bound_var(Regs + tmp2[1], *tmp1); */ make_bound_var_val(Regs + tmp2[1], tag(*tmp1), *tmp1);
			tmp2 += 2;
		} while (--ti > 0);
		pc = tmp2;
		goto LOOP;
	caseop(PUTUVAL):
		...
		if (is_unbound_var(*tmp1)) {
			...
			else /* NEWTAG make_bound_var(Regs + *(pc + 2), *tmp1); */ make_bound_var_val(Regs + *(pc + 2), tag(*tmp1), *tmp1);
			pc = pc + 3;
			goto LOOP;
		}
	caseop(PUTTVAR):
	...
	caseop(PUTTVAL):
#ifdef BIGHEAP
		make_bound_var_val(&(Regs[*(pc + 2)]), tag(Regs[*(pc + 1)]), Regs[*(pc + 1)]);
#else
		Regs[*(pc + 2)] = Regs[*(pc + 1)];
#endif
		pc = pc + 3;
		goto LOOP;
	caseop(PUTSTR):
		/* NEWTAG *heaptop = addtag(TAG_STR, *(pc + 1)); */ addtag(heaptop, TAG_STR, *(pc + 1));
		make_bound_var(Regs + *(pc + 2), heaptop);
		s = ++heaptop;
		pc = pc + 3;
		goto LOOP;
	caseop(PUTLIST):
		/* NEWTAG Regs[*(pc + 1)] = addtag(TAG_CONS, heaptop); */ addtag(&(Regs[*(pc + 1)]), TAG_CONS, heaptop);
		s = heaptop;
		pc += 2;
		goto LOOP;
	caseop(PUTCON):
		/* NEWTAG Regs[*(pc + 2)] = addtag(TAG_CON, *(pc + 1)); */ addtag(&(Regs[*(pc + 2)]), TAG_CON, *(pc + 1));
		pc = pc + 3;
		goto LOOP;
	caseop(PUTNIL):
		/* NEWTAG Regs[*(pc + 1)] = addtag(TAG_NIL, EMPTYLIST); */ addtag(&(Regs[*(pc + 1)]), TAG_NIL, EMPTYLIST);
		pc = pc + 2;
		goto LOOP;
	caseop(BLDPVAR):
		make_unbound_var(s);
	...
	caseop(BLDPVAL):
		...
		if (is_unbound_var(*tmp1)) {
			...
			make_bound_var(s, tmp1);
		else /* NEWTAG make_bound_var(s, *tmp1); */ make_bound_var_val(s, tag(*tmp1), *tmp1);
		heaptop = ++s;
		pc += 2;
		goto LOOP;
	caseop(BLDPVALN):
		...
		for (; ti > 0; ti--) {
			...
			else /* NEWTAG make_bound_var(s+ti, *tmp2); */ make_bound_var_val(s+ti, tag(*tmp2), *tmp2);
		}
		heaptop = s = s + pc[1] + 1;
		pc = pc + pc[1] + 2;			
		goto LOOP;
	...
	caseop(BLDNIL):
		...
		goto LOOP;
	caseop(BLDLIST):
		/* NEWTAG *s = addtag(TAG_CONS, s + 1); */ addtag(s, TAG_CONS, s + 1);
		heaptop = ++s;
		pc++;
		goto LOOP;
	caseop(BLDSTR):
	...
	caseop(SAVE):
		...
		stacktop = curar + *(pc + 1);
#ifdef BIGHEAP
		make_bound_var(curar + FATHER, (int) tmp1);
		make_bound_var(curar + RETPT, (int) retreg);
#else
	...
	caseop(TRY):
		...
		tmp1 = stacktop;
#ifdef BIGHEAP
		make_bound_var(++tmp1, slack_id);
		make_bound_var(++tmp1, solver_id);
		make_bound_var(++tmp1, *(pc + 2)); ti = *(pc + 2);
		make_bound_var(++tmp1, (int) curar);
		make_bound_var(++tmp1, (int) retreg);
		make_bound_var(++tmp1, trtop);
		make_bound_var(++tmp1, (int) heaptop);
		make_bound_var(++tmp1, (int) lastcp);
		make_bound_var(++tmp1, (int) (pc + 3));
#else
		*(++tmp1) = slack_id;
		*(++tmp1) = solver_id;
		*(++tmp1) = ti = *(pc + 2);
	...
	caseop(SAVECURCP):
#ifdef BIGHEAP
		make_bound_var(&(Regs[pc[1]]), (int) lastcp);
#else
	...
	caseop(SAVEPREVCP):
#ifdef BIGHEAP
		make_bound_var(&(Regs[pc[1]]), lastcp[PREVCP]);
#else
	...
	caseop(SAVECUT):
#ifdef BIGHEAP
		make_bound_var_val(curar + pc[2], tag(Regs[pc[1]]), (int) Regs[pc[1]]);
#else
	...
	caseop(DIRSAVECUT):
#ifdef BIGHEAP
		make_bound_var(curar + *(pc + 1), (int) lastcp);
#else
	...
	case (unsigned int) ADD1PFVAR: 
		tmp1 = curar + *(pc + 1);
		/* NEWTAG *tmp1 = addtag(TAG_PAR, ++solver_id); */ addtag(tmp1, TAG_PAR, ++solver_id);
		push_check_trail(BTK_BIND, tmp1);
	...
	caseop(PUTPNUM):
		...
		*(((int *) tfp) + 1) = *(pc + 2);
		...
		/* NEWTAG curar[*(pc + 3)] = addtag(TAG_PAR, solver_id); */ addtag(&(curar[*(pc + 3)]), TAG_PAR, solver_id);
		pc += 4;
		goto LOOP;
	caseop(PUTTNUM):
		*((int *) tfp) = *(pc + 1);
		*(((int *) tfp) + 1) = *(pc + 2);
		makenum(++solver_id, tf);
		/* NEWTAG Regs[*(pc + 3)] = addtag(TAG_PAR, solver_id); */ addtag(&(Regs[*(pc + 3)]), TAG_PAR, solver_id);
		pc += 4;
		goto LOOP;
	caseop(GETPNUM):
		*((int *) tfp) = *(pc + 1);
	...
	caseop(BLDNUM):
		...
		*(((int *) tfp) + 1) = *(pc + 2);
		makenum(++solver_id, tf);
		/* NEWTAG *(s++) = addtag(TAG_PAR, solver_id); */ addtag((s++), TAG_PAR, solver_id);
		heaptop++;
		pc += 3;
		goto LOOP;
	caseop(UNINUM):
	...
	caseop(MVF):
		...
		ti1 = *(pc + 2);
#ifdef BIGHEAP
		make_bound_var_val(Regs + ti1, tag(*(Regs + ti)), *(Regs + ti));
		ti1++; ti++;
		make_bound_var_val(Regs + ti1, tag(*(Regs + ti)), *(Regs + ti));
#else
		*(Regs + ti1++) = *(Regs + ti++);
	...
	caseop(STF):
		...
		tmp1 = curar + *(pc + 2);
#ifdef BIGHEAP
		make_bound_var_val(tmp1, TAG_NUM, 0); tmp1++;
		make_bound_var_val(tmp1, tag(*(Regs + ti)), *(Regs + ti)); ti++; tmp1++;
		make_bound_var_val(tmp1, tag(*(Regs + ti)), *(Regs + ti));
#else
	...
	caseop(LDF):
		...
		tmp1 = curar + *(pc + 1) + 1;
#ifdef BIGHEAP
		make_bound_var_val(Regs + ti, tag(*tmp1), *tmp1); ti++; tmp1++;
		make_bound_var_val(Regs + ti, tag(*tmp1), *tmp1);
#else
		*(Regs + (ti++)) = *(tmp1++);
		*(Regs + ti) = *tmp1;
#endif
	...
	caseop(GETF):
		...
		if (tag(*tmp1) == TAG_PAR && is_ground(mask(*tmp1), &c)) {
			...
			/* VIJAY - is BIGHEAP needed here? */
#ifdef BIGHEAP
			make_bound_var_val(tmp2, tag(*(int *) tfp), *(int *) tfp);
			make_bound_var_val(tmp2 + 1, tag(*((int *) tfp + 1)), *((int *) tfp + 1));
#else
			*tmp2 = *((int *) tfp);
			 *(tmp2 + 1) = *(((int *) tfp) + 1);
#endif
	...
	caseop(PUTF):
		...
		tmp2 = Regs + *(pc + 2);
#ifdef BIGHEAP
		make_bound_var_val((int *) tfp, tag(*tmp1), *tmp1);
		make_bound_var_val((int *) tfp + 1, tag(*(tmp1 + 1)), *(tmp1 + 1));
#else
		*((int *) tfp) = *tmp1;
		*(((int *) tfp) + 1) = *(tmp1 + 1);
#endif
		makenum(++solver_id, tf);
		/* NEWTAG *tmp2 = addtag(TAG_PAR, solver_id); */ addtag(tmp2, TAG_PAR, solver_id); 
		pc = pc + 3;
		goto LOOP;
	caseop(DYNAMIC_TRY):
...
#ifdef BIGHEAP
			make_bound_var(&(Regs[arity_of(ti)+1]), (int) lastcp);
#else
...
{
#ifdef BIGHEAP
	switch(tagnum(tag(*(int *) i))) {
		case NUM_BTK_NULL_EQN: restore_null_eqn(mask(*(int *) i)); break;
		case NUM_BTK_EQN: restore_eqn(pmask(*(int *) i)); break;
		case NUM_BTK_NULL_INEQ: restore_null_ineq(mask(*(int *) i)); break;
		case NUM_BTK_INEQ: restore_ineq(pmask(*(int *) i)); break;
		case NUM_BTK_NLIN: restore_nl_eqn(pmask(*(int *) i)); break;
		case NUM_BTK_DBIND: *((int *)pmask(*(int *) i)) = TAG_DAEMON; break;
		default: fprintf(stderr, "-- %d --\n", tagnum(tag(*(int *) i))); fatal("79980");
	}
#else
	switch (tagnum(i)) {
...
#ifdef BIGHEAP
		switch (tag_pair[(int) tagnum(tag(*t1))][(int) tagnum(tag(*t2))]) {
#else	
...
*/
		...
		case 5: /* 0, TAG_CONS */
#ifdef BIGHEAP
				/* NEWTAG *t1 = *t2 */ make_bound_var_val(t1, tag(*t2), *t2);
#else
				*t1 = *t2;
#endif
		...
		case 11: /* TAG_CONS, 0 */
#ifdef BIGHEAP
				/* NEWTAG *t2 = *t1 */ make_bound_var_val(t2, tag(*t1), *t1);
#else
				*t2 = *t1;
#endif
		...
		case 16: /* TAG_STR, TAG_STR */
				...
				if ((a = mask(*t1)) == (b = mask(*t2))) {
				  ...
				  /* NEWTAG *t1 = t2; */ make_bound_var(t1, t2);
#endif
					...
					do {
#ifdef BIGHEAP
						make_bound_var(&unifystack[++top], ++t1);
						make_bound_var(&unifystack[++top], ++t2);
#else
						stack[++top] = ++t1;
						stack[++top] = ++t2;
#endif
		...
		case 17: /* TAG_CONS, TAG_CONS */
				if (*t1 != *t2) {
					...
					t2 = (int *) pmask(*t2);
#ifdef BIGHEAP
					make_bound_var(&unifystack[++top], t1); t1++;
					make_bound_var(&unifystack[++top], t2); t2++;
					make_bound_var(&unifystack[++top], t1);
					make_bound_var(&unifystack[++top], t2);
#else
					stack[++top] = t1++;
...
*/
	...
	while (r) {
		...
		addtag(&t2, TAG_HEAPIFY_PTERM, t2);
		TAG_HEAPIFY_PTERM = 0;
...
*/
	...
	while (r) {
		if (r->endcode == -1) goto MORE;
#ifdef BIGHEAP
		splmem[SPLMEM_RETRACT_RULE_T2] = pterm_to_heap(r->t);
		addtag(&(splmem[SPLMEM_RETRACT_RULE_T2]), TAG_HEAPIFY_PTERM, splmem[SPLMEM_RETRACT_RULE_T2]);
		TAG_HEAPIFY_PTERM = 0;
		if (unify(t, &(splmem[SPLMEM_RETRACT_RULE_T2]))) {
#else
		t2 = pterm_to_heap(r->t);
		if (unify(t, &t2)) {
#endif
...
**/	
	...
	while (r) {
...
#ifdef BIGHEAP
		splmem[SPLMEM_HEAD_HTERM] = pterm_to_quoted_heap(head_pterm, FALSE);
		addtag(&splmem[SPLMEM_HEAD_HTERM], TAG_HEAPIFY_PTERM, splmem[SPLMEM_HEAD_HTERM]);
		TAG_HEAPIFY_PTERM = 0;
		if (unify(head, &(splmem[SPLMEM_HEAD_HTERM]))) {
#else
		...
		if (unify(head, &head_hterm)) {
			...
			if (body_pterm) {
#ifdef BIGHEAP
				splmem[SPLMEM_BODY_HTERM] = pterm_to_quoted_heap(body_pterm, TRUE);
				addtag(&splmem[SPLMEM_BODY_HTERM], TAG_HEAPIFY_PTERM, splmem[SPLMEM_BODY_HTERM]);
				TAG_HEAPIFY_PTERM = 0;
#else
...
#ifdef BIGHEAP
				 make_bound_var(&(splmem[SPLMEM_BODY_HTERM]), &(splmem[SPLMEM_TRUE_ATOM]));
#else
				 body_hterm = &true_atom;
#endif
			}
#ifdef BIGHEAP
			if (!unify(body, &(splmem[SPLMEM_BODY_HTERM]))) goto UNDO;
#else
...
int i, t;
	for (i = trtop - 1; i >= new_trtop; i--) {
		...
		// Vijay - for some reason when tag is BTK_BIND crash occurs if
		// the 4 bits are restored from tagtrail. Have to make sure the reason.
		if (tag(trail[i]) == BTK_BIND) make_unbound_var((int *) (pmask(t)));// | ((unsigned)tagtrail[i] << 28)));
#ifdef BIGHEAP
		else solver_backtrack(i, &trail[i]);
#else
...
double fp_val;
	...
	if (first) { /*** pseudo-indexing ***/
		...
		for (i = 1; i <= n; i++) {
#ifdef BIGHEAP
			make_bound_var_val(&(lastcp[i]), tag(regs[i]), regs[i]);
#else
			lastcp[i] = regs[i];
#endif
		}
...
int *t, val;
{
	t = findbind(t);
	if (is_unbound_var(*t)) {
		/* NEWTAG *t = addtag(TAG_CON, val); */ addtag(t, TAG_CON, val);
...
int *t;
{
	t = findbind(t);
	if (is_unbound_var(*t)) {
		/* NEWTAG *t = addtag(TAG_NIL, EMPTYLIST); */ addtag(t, TAG_NIL, EMPTYLIST);
		push_check_trail(BTK_BIND, t);
		return TRUE;
	} else if (is_daemon(t)) {
		/* NEWTAG *t = addtag(TAG_NIL, EMPTYLIST); */ addtag(t, TAG_NIL, EMPTYLIST);
...
double c;
	...
	if (is_unbound_var(*t)) {
		...
		/* NEWTAG *t = addtag(TAG_PAR, solver_id); */ addtag(t, TAG_PAR, solver_id);
		push_check_trail(BTK_BIND, t);
		return TRUE;
	} else if (is_daemon(t)) {
		makenum(++solver_id, val);
...
int *h, i, eflag=FALSE;
	...
	/* NEWTAG make_bound_var(h, addtag(TAG_STR, val)) */ make_bound_var_val(h, TAG_STR, val); 
	for (i=0; i < arity; i++)
...
int *h, eflag=FALSE;
	...
	h = heaptop;
	heaptop += 2;
	/* NEWTAG make_bound_var(t, addtag(TAG_CONS, h)) */ make_bound_var_val(t, TAG_CONS, h); 
	make_unbound_var(h);
...
#ifdef BIGHEAP
		make_bound_var(&(trail[trtop++]), (int *) val);
        addtag(&(trail[trtop++]), BTK_TEST2, (int *) t);
#else
...
inline addtag_func(l, tag, r)
unsigned int l, tag, r;
{
	if(is_stackaddr(l)) {
		memtags[stacktags_index(l)] = (unsigned) tag >> 28;
		*(int *) l = r;
		return;
	}
	if(is_trailaddr(l)) {
		memtags[trailtags_index(l)] = (unsigned) tag >> 28;
		*(int *) l = r;
		return;
	}
	if(is_unifystackaddr(l)) {
		unifytags[unifytags_index(l)] = (unsigned) tag >> 28;
		*(int *) l = r;
		return;
	}
	if(is_splmemaddr(l)) {
		spltags[spltags_index(l)] = (unsigned) tag >> 28;
		*(int *) l = r;
		return;
	}
	fatal("addtag called with invalid address");
}

/*
	Vijay - testing for performance. risky!
*/
inline addtagcmp_func(l, tag, r)
unsigned int l, tag, r;
{
	return 
		is_stackaddr(l)? 		((((unsigned) memtags[stacktags_index(l)] << 28) == (unsigned) tag) && (* (int *)l == r)):
		is_trailaddr(l)? 		((((unsigned) memtags[trailtags_index(l)] << 28) == (unsigned) tag) && (* (int *)l == r)):
		is_unifystackaddr(l)?	((((unsigned) unifytags[unifytags_index(l)] << 28) == (unsigned) tag) && (* (int *)l == r)):
		is_splmemaddr(l)?	 	((((unsigned) spltags[spltags_index(l)] << 28) == (unsigned) tag) && (* (int *)l == r)):
		fatal("addtagcmp called with invalid address");
}
...
unsigned int i;
{
	...
	return 
		is_stackaddr(i)? 		(unsigned) memtags[stacktags_index(i)] << 28:
		is_trailaddr(i)? 		(unsigned) memtags[trailtags_index(i)] << 28:
		is_unifystackaddr(i)?	(unsigned) unifytags[unifytags_index(i)] << 28:
		is_splmemaddr(i)?		(unsigned) spltags[spltags_index(i)] << 28:
		fatal("tag called with invalid address");
}

inline make_bound_var_addr_func(i, j)
unsigned int i;
int j;
{
	if(is_stackaddr(i)) {
		memtags[stacktags_index(i)] = 0;
		* (int *) i = j;
		return;
	}
	if(is_trailaddr(i)) {
		memtags[trailtags_index(i)] = 0;
		* (int *) i = j;
		return;
	}
	if(is_unifystackaddr(i)) {
		unifytags[unifytags_index(i)] = 0;
		* (int *) i = j;
		return;
	}
	if(is_splmemaddr(i)) {
		spltags[spltags_index(i)] = 0;
		* (int *) i = j;
		return;
	}
	fatal("make_bound_var called with invalid address");
}

inline make_bound_var_val_func(i, tag, j)
unsigned int i;
int tag;
int j;
{
	if(is_stackaddr(i)) {
		memtags[stacktags_index(i)] = (unsigned) tag >> 28;
		* (int *) i = j;
		return;
	}
	if(is_trailaddr(i)) {
		memtags[trailtags_index(i)] = (unsigned) tag >> 28;
		* (int *) i = j;
		return;
	}
	if(is_unifystackaddr(i)) {
		unifytags[unifytags_index(i)] = (unsigned) tag >> 28;
		* (int *) i = j;
		return;
	}
	if(is_splmemaddr(i)) {
		spltags[spltags_index(i)] = (unsigned) tag >> 28;
		* (int *) i = j;
		return;
	}
	fatal("make_bound_var_val called with invalid address");
}

inline make_unbound_var_func(i)
int i;
{
	if(is_stackaddr(i)) {
		memtags[stacktags_index(i)] = NULL;
		* (int *) i = NULL;
		return;
	}
	if(is_trailaddr(i)) {
		memtags[trailtags_index(i)] = NULL;
		* (int *) i = NULL;
		return;
	}
	if(is_unifystackaddr(i)) {
		unifytags[unifytags_index(i)] = NULL;
		* (int *) i = NULL;
		return;
	}
	if(is_splmemaddr(i)) {
		spltags[spltags_index(i)] = NULL;
		* (int *) i = NULL;
		return;
	}
	fatal("make_unbound_var called with invalid address");
}
