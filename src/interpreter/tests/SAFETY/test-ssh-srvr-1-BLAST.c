...
{ 
...

  {
    ...
    NONDET = unknown();
    if (NONDET) /* ((unsigned long )s->cert == (unsigned long )((void *)0)) */ {
	...
	/* } */
	NONDET=unknown(); 
	if (NONDET) /* ((s->s3)->tmp.use_rsa_tmp) */ {
	  goto _L___0;
	} else {
	  if (l & 30UL) {
	    goto _L___0;
	    ...
	    if (l & 1UL) { 
	      NONDET=unknown(); 
	      if (NONDET) /* ((unsigned long )(s->cert)->pkeys[0].privatekey == (unsigned long )((void *)0)) */ {
		goto _L___0;
	      } else {
		NONDET=unknown(); 
		if (NONDET) /* (((s->s3)->tmp.new_cipher)->algo_strength & 2UL) */ { 
		  //tmp___6 = EVP_PKEY_size((s->cert)->pkeys[0].privatekey); 
		  tmp___6 = unknown();
		  NONDET=unknown(); 
		  if (NONDET) /* (((s->s3)->tmp.new_cipher)->algo_strength & 4UL) */ { 
		    tmp___7 = 512;
		  } else {
		    tmp___7 = 1024;
		  }
	...
	break;
      ...
      case 8545: ;
	NONDET=unknown(); 
	if (NONDET) /*(s->verify_mode & 1)*/ { 
	  NONDET=unknown(); 
	  if (NONDET) /* ((unsigned long )(s->session)->peer != (unsigned long )((void *)0)) */ { 
	    NONDET=unknown(); 
	    if (NONDET) /* (s->verify_mode & 4) */ {
	      skip = 1;
	...
	else if(blastFlag == 5) goto ERROR; //if(blastFlag == 5) _TRACER_abort(blastFlag == 5); 
	if (ret <= 0) {
	  goto end;
	}
	myState /*s->state*/ = 8448;
