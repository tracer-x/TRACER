...
int main() {
  ...
  if (BLAST_NONDET) c = 0; else c = 1;
  p();
  // BLAST
  if(x0+x1+x2+x3+x4+x5+x6+x7+x8+x9 == 33 && c == 0) ERROR:goto ERROR;
  // TRACER _ABORT(x0+x1+x2+x3+x4+x5+x6+x7+x8+x9 == 33 && c == 0);
...
void p() {
	...
	if (BLAST_NONDET) x5=x5+2; else x5=x5+1; 
