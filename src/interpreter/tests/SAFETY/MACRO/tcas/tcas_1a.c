...
unsigned alt_sep_test()
{
    ...
    if (enabled && ((tcas_equipped && intent_not_known) || !tcas_equipped))
    {
      ...
      if (need_upward_RA && need_downward_RA)
        ...
        {
	  /*property1b(alim) ;*/
	  //_TRACER_abort(Up_Separation < alim && Down_Separation >= alim);
	  /*property2b(alim) ;*/
	  //	  _TRACER_abort(Up_Separation < alim && Down_Separation < alim && Up_Separation < Down_Separation);
	  /*property3b(alim) ;*/
	  //	  _TRACER_abort(Up_Separation >= alim && Down_Separation >= alim && Own_Tracked_Alt > Other_Tracked_Alt);
	  /*property4b() ;*/
	  //	  _TRACER_abort(Own_Tracked_Alt < Other_Tracked_Alt);
	  /*property5b() ;*/
	  //	  _TRACER_abort(Up_Separation < Down_Separation);	 
	  alt_sep = 1;
        }
	  ...
	  // TRACER
	  _TRACER_abort( Up_Separation >= alim && Down_Separation < alim );                
	  //_DECOMPILE_TRACER_abort( Up_Separation >= alim && Down_Separation < alim );                
	  /*property2a(alim) ;*/
	  //	  _TRACER_abort(Up_Separation < alim && Down_Separation < alim && Up_Separation > Down_Separation);
	  /*property3a(alim) ;*/
	  //          _TRACER_abort(Up_Separation >= alim && Down_Separation >= alim && Own_Tracked_Alt > Other_Tracked_Alt);
