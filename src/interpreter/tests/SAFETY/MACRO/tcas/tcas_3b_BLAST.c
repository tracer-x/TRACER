...
unsigned alt_sep_test()
{
    ...
    if (enabled && ((tcas_equipped && intent_not_known) || !tcas_equipped))
    {
	  ...
	  /*property3b(alim) ;*/
	  if(Up_Separation >= alim && Down_Separation >= alim && Own_Tracked_Alt > Other_Tracked_Alt) ERROR: goto ERROR;
	  //_ABORT(Up_Separation >= alim && Down_Separation >= alim && Own_Tracked_Alt > Other_Tracked_Alt);
	  //_DECOMPILE_ABORT(Up_Separation >= alim && Down_Separation >= alim && Own_Tracked_Alt > Other_Tracked_Alt);
	  /*property4b() ;*/
