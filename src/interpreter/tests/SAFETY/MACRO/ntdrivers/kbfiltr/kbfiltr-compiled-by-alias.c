...
typedef BOOLEAN ( *  PFAST_IO_READ_COMPRESSED )(struct  _FILE_OBJECT *
	...
	/*struct _COMPRESSED_DATA_INFO*/ int *  CompressedDataInfo , ULONG
...
typedef void (/*_fastcall*/ *  pHalIoAssignDriveLetters
	)(/*struct _LOADER_PARAMETER_BLOCK*/ int *  LoaderBlock , PSTRING NtDeviceName ,
...
typedef struct myIRP_{
  NTSTATUS Status;
} myIRP;

IRP *pirp ; 
// TRACER: we don't compile IRP since it's not standard ANSI C
myIRP *mypirp;
// BLAST: to shadow (pirp->IoStatus).Status without pointers
//int myStatus ;
int s ;
					...
					PBOOLEAN TurnTranslationOn )
{
  ...
  NTSTATUS status = (NTSTATUS ) 0 /*0x00000000L*/;
...
NTSTATUS KbFilter_CreateClose(PDEVICE_OBJECT DeviceObject , PIRP Irp )
{
	...
	// BLAST shadow
	status = mypirp->Status /*(Irp->IoStatus).Status*/;
	//status = myStatus;
	...
	mypirp->Status /*(Irp->IoStatus).Status*/ = status;
	// BLAST shadow
	//myStatus = status;
...
/* 	}	 */
	...
	if((NTSTATUS )status >= 0 && (NTSTATUS )mypirp->Status /*(Irp->IoStatus).Status*/ /*myStatus*/ >= 0)
	  {
	    devExt->Started = 1;
	    devExt->Removed = 0;
	    devExt->SurpriseRemoved = 0;
...
*/
  ...
  switch(BLAST_NONDET)
    {
    case 0:
      returnVal = (NTSTATUS ) 0 /*0x00000000L*/;
      break;
    case 1:
...
int main()
{
  ...
  pirp = &irp;
  _BLAST_init();        
  status = (NTSTATUS) 0; /* DriverEntry(& d, & u); DriverEntry always
                            returns 0 d and u are modified but safety
                            does not depend on them. Thus we can
                            abstract here. */
  ...
  if((NTSTATUS )status >= 0)
    {     
      ...
      mypirp->Status  /*(pirp->IoStatus).Status*/ = (NTSTATUS )0x00000000L;
      //BLAST shadow
      // myStatus = (NTSTATUS ) 0 /*0x00000000L*/;
      if(irp_choice == 0)
	{
	  mypirp->Status /*(pirp->IoStatus).Status*/ = (NTSTATUS )0xC00000BBL;
	  //BLAST shadow
