...
/* 
  ...
  Also, there are a couple of unbonded loops through goto's. TRACER
  does not force loop invariants with goto's. Therefore, termination
  is not ensured if they are executed.
...
NTSTATUS CdAudioStartDevice(PDEVICE_OBJECT DeviceObject , PIRP Irp )
{
  ...
  if(deviceExtension->Active == (UCHAR )0xFF)
      ...
      if(! inquiryDataPtr)
	{
      ...
      /* 	{ */
      /* 	  // The loop is executed at most 3 times */
      /* 	  attempt++; */
      ...
      /* 	} */
      /*
       TRACER: since the number of iterations is bounded to at most 3
       times we just force the loop unwinding
      */
    LOOP1: 
      if(! ((NTSTATUS )status>= 0) && attempt < 4)
      	{
      	  attempt++;
      	  memset(& srb, 0, sizeof(SCSI_PASS_THROUGH ));
      	  memset(inquiryDataPtr, 0, 36);
...
NTSTATUS CdAudioPioneerDeviceControl(PDEVICE_OBJECT DeviceObject , PIRP Irp )
{
 ...
 PioneerRestart:
  ...
  switch(NONDET/* ((currentIrpStack->Parameters).DeviceIoControl).IoControlCode */)
      ...
      if(temp_1)
	{
      ...
      Toc = (PUCHAR)ExAllocatePoolWithTag(NonPagedPoolCacheAligned,sizeof(CDROM_TOC ),tag_1 
					  /* ' AdC' */);
      if(Toc == (void *  )0)
	{
	  status = (NTSTATUS )0xC000009AL;
	...
	tracksToReturn =tracksOnCd;
      ...
      /* 
	 TRACER 
	 This loop is unbounded so we need MINMAX
      */
 ...
 (Irp->IoStatus).Information = (ULONG )((LONG)((LONG_PTR )(& (((CDROM_TOC *)0)->TrackData)[i])));   
	 ...
	 sizeof(CDROM_PLAY_AUDIO_MSF ))
	{
      ...
      /* 
	 TRACER: since the loop is bounded (at most 5 times) we can
	...
	status = SendSrbSynchronous(deviceExtension, & srb, SubQPtr, 6);
      ...
      if(! ((NTSTATUS )status >= 0) && retry > 0 && status != (NTSTATUS )0xC00000A3L){
	retry--;
	goto LOOPX4;
      }
      if((NTSTATUS )status >= 0)
	...
	if(currentIrpStack->Flags & 0x02)
	  {
	    status = (NTSTATUS )0xC0000185L;
	    /*
	      TRACER warning: if executed this goto it may go through
	      an infinite loop
	     */
	    goto PioneerRestart;
	  }
	...
	if(currentIrpStack->Flags & 0x02)
	  {
	    ...
	    /* 
               TRACER warning: if this goto is executed we may go
 ...
 HitachiRestart:
  ...
  switch(NONDET /* ((currentIrpStack->Parameters).DeviceIoControl).IoControlCode */)
	...
	if((NTSTATUS )status >= 0 || status == (NTSTATUS)0xC000003CL)
	  ...
	  {
	    // TRACER warning: if this is executed we DO go through an
	    // infinite loop
	    //if((SubQPtr[1] & 0x0F) != 1)
	    //  goto Retry;
 ...
 SetStatusAndReturn:
  {
	...
	if(currentIrpStack->Flags & 0x02)
	  {
	    status = (NTSTATUS )0xC0000185L;
	    // TRACER warning: if this is executed we may go to an
	    // infinite loop
	    goto HitachiRestart;
	  }
	IoSetHardErrorOrVerifyDevice(Irp, deviceExtension->TargetDeviceObject);
...
PIRP IoBuildDeviceIoControlRequest(ULONG IoControlCode ,
				   PDEVICE_OBJECT DeviceObject , PVOID InputBuffer , ULONG InputBufferLength ,
...
NTSTATUS IoQueryDeviceDescription(PINTERFACE_TYPE BusType,
				  PULONG BusNumber , PCONFIGURATION_TYPE ControllerType , PULONG ControllerNumber ,
				  PCONFIGURATION_TYPE PeripheralType , PULONG
				  PeripheralNumber , PIO_QUERY_DEVICE_ROUTINE CalloutRoutine , 
				  PVOID Context )
{
  int BLAST_NONDET ;
  switch(BLAST_NONDET)
