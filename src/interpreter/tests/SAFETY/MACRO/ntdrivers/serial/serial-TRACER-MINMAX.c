/* 
   ...
   fully reason about pointers.
*/

/*
  Read comments in tag "Problem" in function SerialStartRead 
 */
...
int customIrp ;
int routine ;
/* TRACER: to shadow pointers */
int myStatus ;
int myArgument4;
/* TRACER: to shadow NextIrp in several functions*/
int myNextIrpFlag; 
/* TRACER: to shadow PIRP*  CurrentOpIrp in SerialGetNextIrp*/
int myCurrentOpIrpFlag;
/* TRACER: to simulate unknown value */
extern int unknown();
...
NTSTATUS SerialStartFlush(PSERIAL_DEVICE_EXTENSION Extension )
{
  ...
  if(myNextIrpFlag)  /*TRACER: to shadow NewIrp */
    SerialStartWrite(Extension);

  myNextIrpFlag = unknown(); // to avoid propagate beyond this function
			    ...
			    PSERIAL_DEVICE_EXTENSION Extension )
{
  ...
  * CurrentOpIrp = (void *  )0;
  * NewIrp = (void *  )0;
  /*TRACER: to shadow * NewIrp */
  myNextIrpFlag = (void *  )0; 
  ...
  do  
  {
	...
	* & OldIrql = KfAcquireSpinLock(& Extension->ControlLock);
    ...
    SerialGetNextIrp(& Extension->CurrentPurgeIrp, 
		     & Extension->PurgeQueue,
		     & NewIrp, 1, Extension);
  } while(myNextIrpFlag /*TRACER: to shadow NewIrp*/); 
  myNextIrpFlag = unknown(); // to avoid propagate beyond this function 
  return (NTSTATUS )0x00000000L;
}
...
NTSTATUS SerialStartRead(PSERIAL_DEVICE_EXTENSION Extension )
{
  ...
  /*
    Problem: we have observed that if the loop is executed twice the
   ...
   */
  myCurrentOpIrpFlag = 1; // TRACER: & Extension->CurrentReadIrp is not null
  do   
  {
			    ...
			    KIRQL OldIrql )
{
  ...
  oldIrp = * CurrentOpIrp;
  // TRACER: shadow * CurrentOpIrp;
  myoldIrp = myCurrentOpIrpFlag;
  ...
  if(! (QueueToProcess->Flink == QueueToProcess))
    {
      ...
      EX_Blink->Flink = EX_Flink;
      EX_Flink->Blink = EX_Blink;
      * CurrentOpIrp = (IRP *)((PCHAR )headOfList - (ULONG_PTR )(& ((((IRP *  )0)->Tail).Overlay).ListEntry));
      (PDRIVER_CANCEL )((PVOID )InterlockedExchange((PLONG )((PVOID * )(& (* CurrentOpIrp)->CancelRoutine)), 
						    (LONG )((PVOID)((void *  )0))));  
      // TRACER: shadow *CurrentOpIrp
      // TRACER problem: if myCurrentOpIrpFlag = unknown(); then
      //                 program is unsafe
      myCurrentOpIrpFlag = 0 ; 
    }
  else{
    * CurrentOpIrp    = (void *  )0;
    // TRACER: shadow *CurrentOpIrp
  ...
  * NextIrp = * CurrentOpIrp;
  // TRACER: shadow *NextIrp
  myNextIrpFlag = myCurrentOpIrpFlag;  

  IoReleaseCancelSpinLock(OldIrql);

  if(CompleteCurrent)
    {
      if(myoldIrp /*TRACER: oldIrp*/)  
	{
	  IofCompleteRequest(oldIrp, 2);
	  SerialIRPEpilogue(extension);
	}
				...
				LONG RefType )
{
  ...
  if(! myArgument4 
      ...
      if(StatusToUse == (NTSTATUS )0xC0000120L)
	{
      ...
      if(GetNextRoutineCode)  // (GetNextIrp)
	{
	  IoReleaseCancelSpinLock(IrqlForRelease);
	  // TRACER: to similate function calls to GetNextIrp
	  // GetNextIrp(CurrentOpIrp, QueueToProcess, & newIrp, 1, Extension);
	  switch(GetNextRoutineCode)
	    {
	    case 1 /*SerialGetNextImmediate*/: 
	      SerialGetNextImmediate(CurrentOpIrp, QueueToProcess, & newIrp, 1, Extension);
	      break;
	    case 2 /*SerialGetNextIrp */     : 
	      SerialGetNextIrp(CurrentOpIrp, QueueToProcess, & newIrp, 1, Extension);
	      break;
	    case 3 /*SerialGetNextWrite */   : 
	      SerialGetNextWrite(CurrentOpIrp, QueueToProcess, & newIrp, 1, Extension);
	      break;
	    default: 
	      break;
	    }
...
NTSTATUS SerialStartMask(PSERIAL_DEVICE_EXTENSION Extension )
{
  ...
  do 
	 ...
	 (0x0000001b << 16 | 0 << 14 | 17 << 2 | 0))
	{
	  ...
	  ((Extension->CurrentMaskIrp)->IoStatus).Status = (NTSTATUS )0x00000000L;
	  if(! SetFirstStatus)
	    {
	      FirstStatus = (NTSTATUS )0x00000000L;
	      SetFirstStatus = 1;
	    }
	  SerialGetNextIrp(& Extension->CurrentMaskIrp, & Extension->MaskQueue,
			   & NewIrp, 1, Extension);
	}
      else
	{
	  if(! Extension->IsrWaitMask || Extension->CurrentWaitIrp)
		...
		{
		  IoReleaseCancelSpinLock(OldIrql);
		  ((Extension->CurrentMaskIrp)->IoStatus).Status = (NTSTATUS )0xC0000120L;
		  if(! SetFirstStatus)
		    {
		      FirstStatus = (NTSTATUS)0xC0000120L;
		      SetFirstStatus = 1;
...
/* 		    } */
/* 		  while(0); */
		  KeSynchronizeExecution(Extension->Interrupt,(PKSYNCHRONIZE_ROUTINE) 0/*SerialGiveWaitToIsr*/,
					 Extension);
		  Extension->CurrentMaskIrp = (void *  )0;
		  // TRACER: shadow & Extension->CurrentMaskIrp
		  myCurrentOpIrpFlag = 0;
		  SerialGetNextIrpLocked(& Extension->CurrentMaskIrp,
					 & Extension->MaskQueue, 
					 & NewIrp, 0, Extension, OldIrql); 
		}
	    }
	}
    }  while(myNextIrpFlag /*TRACER: to shadow NewIrp*/); 
  ...
  while(1)
	...
	MmLockPagableSectionByHandle(SerialGlobals.PAGESER_Handle);
	 ...
	 TRACER: This function changes many global variables. Without
         MINMAX we may report false positives
      */
