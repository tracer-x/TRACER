...
struct ssl3_enc_method;struct ssl_method_st {
   ...
   int (*ssl_version)() ;
  // The TRACER compiler has problems with the following lines:
...
int main()
{
  ...
  mystate = 0x3004; // (0x04|(0x1000|0x2000)); 12292 in decimal 
  ...
  { 
	...
	switch (mystate) { // TRACER shadows s->state
	case 12292:
	  s->new_session = 1;
	  s->state = 4096;   // TRACER shadows s->state
	  mystate = 4096;
	...
	tmp___4 = TRACER_NONDET;
	if (! tmp___4) { 
	  ret = -1;
	  goto end;
	}
	//BLAST tmp___5 = ssl_init_wbio_buffer(s, 0); 
	tmp___5 = TRACER_NONDET;
	if (! tmp___5) { 
	  ret = -1;
	  goto end;
	}
	...
	s->state = 4368;
	mystate = 4368; // TRACER shadows s->state
	...
	s->state = 4384;
	mystate = 4384; // TRACER shadows s->state
	s->init_num = 0;
	...
	if (s->hit) { 
	  s->state = 4560;
	  mystate = 4560; // TRACER shadows s->state
	} else {
	  s->state = 4400;
	  mystate = 4400; // TRACER shadows s->state
	}
	s->init_num = 0;
	break;
	case 4400: ;
	case 4401: ;
	  ...
	  s->state = 4416;
	  mystate = 4416; // TRACER shadows s->state
	  s->init_num = 0;
	  break;
	case 4416: ;
	case 4417:
	...
	s->state = 4432;
	mystate = 4432; // TRACER shadows s->state
	s->init_num = 0;
	//BLAST tmp___6 = ssl3_check_cert_and_algorithm(s); 
	tmp___6 = TRACER_NONDET;
	if (! tmp___6) {
	  ret = -1;
	  goto end;
	}
	...
	s->state = 4448;
	mystate = 4448; // TRACER shadows s->state
	s->init_num = 0;
	break;
	case 4448: ;
	case 4449:
	...
	if ((s->s3)->tmp.cert_req) { 
	  s->state = 4464;
	  mystate = 4464; // TRACER shadows s->state
	} else {
	  s->state = 4480;
	  mystate = 4480; // TRACER shadows s->state
	}
	s->init_num = 0;
	break;
	case 4464: ;
	...
	if ((s->s3)->tmp.cert_req == 1) {
	  s->state = 4496;
	  mystate = 4496; // TRACER shadows s->state
	} else {
	  s->state = 4512;
	  mystate = 4512; // TRACER shadows s->state
	...
	s->state = 4528;
	mystate = 4528; // TRACER shadows s->state
	s->init_num = 0;
	...
	s->state = 4352;
	mystate = 4352; // TRACER shadows s->state
	(s->s3)->flags &= -5L;
	if (s->hit) {
	...
	if (s->hit) { 
	  ...
	  s->state = 3;
	  mystate = 3; // TRACER shadows s->state
	}
	s->init_num = 0;
	break;
	case 4352:
	  ...
	  mystate = (s->s3)->tmp.next_state; // TRACER shadows s->state
	  break;
	case 3:
	...
	if (! (s->s3)->tmp.reuse_message) { 
	  if (! skip) {
	    ...
	    if ((unsigned long )cb != (unsigned long )((void *)0)) { 
	      if (mystate != state) {  // TRACER shadows s->state
		new_state = mystate;
		mystate = state;
		//BLAST ((*cb))(s, 4097, 1);
		mystate = new_state;
