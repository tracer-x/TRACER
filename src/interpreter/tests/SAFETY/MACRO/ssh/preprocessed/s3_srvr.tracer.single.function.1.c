...
struct rsa_meth_st {
   char const *name ;
   int (*rsa_pub_enc)(int flen1 , unsigned char *from1 , unsigned char *to1 , RSA *rsa1 ,
                      int padding1 ) ;
   int (*rsa_pub_dec)(int flen2 , unsigned char *from2 , unsigned char *to2 , RSA *rsa2 ,
                      int padding2 ) ;
   int (*rsa_priv_enc)(int flen3 , unsigned char *from3 , unsigned char *to3 , RSA *rsa3 ,
                       int padding3 ) ;
   int (*rsa_priv_dec)(int flen4 , unsigned char *from4 , unsigned char *to4 , RSA *rsa4 ,
                       int padding4 ) ;
   int (*rsa_mod_exp)(BIGNUM *r0 , BIGNUM *I , RSA *rsa ) ;
   int (*bn_mod_exp)(BIGNUM *r , BIGNUM *a , BIGNUM const *p , BIGNUM const *m ,
                     BN_CTX *ctx , BN_MONT_CTX *m_ctx ) ;
   int (*init)(RSA *rsa11 ) ;
   int (*finish)(RSA *rsa12 ) ;
   int flags ;
   char *app_data ;
   int (*rsa_sign)(int type5 , unsigned char *m5 , unsigned int m_len5 , unsigned char *sigret5 ,
                   unsigned int *siglen5 , RSA *rsa5 ) ;
   int (*rsa_verify)(int dtype6 , unsigned char *m6 , unsigned int m_len6 , unsigned char *sigbuf6 ,
                     unsigned int siglen6 , RSA *rsa6 ) ;
};
...
struct dh_method {
   char const *name ;
   int (*generate_key)(DH *dh1 ) ;
   int (*compute_key)(unsigned char *key2, BIGNUM *pub_key2 , DH *dh2 ) ;
   int (*bn_mod_exp)(DH *dh3 , BIGNUM *r3 , BIGNUM *a3 , BIGNUM const *p3 , BIGNUM const *m3 ,
                     BN_CTX *ctx3 , BN_MONT_CTX *m_ctx3 ) ;
   int (*init)(DH *dh4 ) ;
   int (*finish)(DH *dh5 ) ;
   int flags ;
   char *app_data ;
...
struct dsa_method {
   char const *name ;
   DSA_SIG *(*dsa_do_sign)(unsigned char const *dgst1 , int dlen1 , DSA *dsa1 ) ;
   int (*dsa_sign_setup)(DSA *dsa2 , BN_CTX *ctx_in2 , BIGNUM **kinvp2 , BIGNUM **rp2 ) ;
   int (*dsa_do_verify)(unsigned char const *dgst3 , int dgst_len3 , DSA_SIG *sig3 ,
                        DSA *dsa3 ) ;
   int (*dsa_mod_exp)(DSA *dsa4 , BIGNUM *rr4 , BIGNUM *a14 , BIGNUM *p14 , BIGNUM *a24 ,
                      BIGNUM *p24 , BIGNUM *m4 , BN_CTX *ctx4 , BN_MONT_CTX *in_mont4 ) ;
   int (*bn_mod_exp)(DSA *dsa5 , BIGNUM *r5 , BIGNUM *a5 , BIGNUM const *p5 , BIGNUM const *m5 ,
                     BN_CTX *ctx5 , BN_MONT_CTX *m_ctx5 ) ;
   int (*init)(DSA *dsa6 ) ;
   int (*finish)(DSA *dsa7 ) ;
   int flags ;
   char *app_data ;
...
struct evp_cipher_st {
   ...
   int iv_len ;
   unsigned long flags ;
   int (*init)(EVP_CIPHER_CTX *ctx1 , unsigned char const *key1 , unsigned char const *iv1 ,
               int enc1 ) ;
   int (*do_cipher)(EVP_CIPHER_CTX *ctx2 , unsigned char *out2 , unsigned char const *in2 ,
                    unsigned int inl2 ) ;
...
struct x509_store_st {
   ...
   int (*verify)(X509_STORE_CTX *ctx1 ) ;
   int (*verify_cb)(int ok , X509_STORE_CTX *ctx2 ) ;
   CRYPTO_EX_DATA ex_data ;
   int references ;
   int depth ;
};
...
struct x509_store_ctx_st {
   ...
   int (*verify)(X509_STORE_CTX *ctx1 ) ;
   int (*verify_cb)(int ok , X509_STORE_CTX *ctx2) ;
   int (*get_issuer)(X509 **issuer3 , X509_STORE_CTX *ctx3 , X509 *x3 ) ;
   int (*check_issued)(X509_STORE_CTX *ctx4 , X509 *x4 , X509 *issuer4 ) ;
   int (*cleanup)(X509_STORE_CTX *ctx5 ) ;
   int depth ;
   int valid ;
   int last_untrusted ;
   STACK *chain ;
...
struct ssl3_enc_method;struct ssl_method_st {
   int version ;
   int (*ssl_new)(SSL *s1 ) ;
   void (*ssl_clear)(SSL *s2 ) ;
   void (*ssl_free)(SSL *s3 ) ;
   int (*ssl_accept)(SSL *s4 ) ;
   int (*ssl_connect)(SSL *s5 ) ;
   int (*ssl_read)(SSL *s6 , void *buf6 , int len6 ) ;
   int (*ssl_peek)(SSL *s7 , void *buf7 , int len7 ) ;
   int (*ssl_write)(SSL *s8 , void const *buf8 , int len8 ) ;
   int (*ssl_shutdown)(SSL *s9 ) ;
   int (*ssl_renegotiate)(SSL *s10 ) ;
   int (*ssl_renegotiate_check)(SSL *s11 ) ;
   long (*ssl_ctrl)(SSL *s12 , int cmd12 , long larg12 , char *parg12 ) ;
   long (*ssl_ctx_ctrl)(SSL_CTX *ctx13 , int cmd13 , long larg13 , char *parg13 ) ;
   SSL_CIPHER *(*get_cipher_by_char)(unsigned char const *ptr14 ) ;
   int (*put_cipher_by_char)(SSL_CIPHER const *cipher15 , unsigned char *ptr15 ) ;
   int (*ssl_pending)(SSL *s16 ) ;
   int (*num_ciphers)(void) ;
   SSL_CIPHER *(*get_cipher)(unsigned int ncipher17 ) ;
   struct ssl_method_st *(*get_ssl_method)(int version18 ) ;
...
struct cert_st;struct ssl_ctx_st {
   ...
   int (*new_session_cb)(struct ssl_st *ssl1 , SSL_SESSION *sess1 ) ;
   void (*remove_session_cb)(struct ssl_ctx_st *ctx2 , SSL_SESSION *sess2 ) ;
   SSL_SESSION *(*get_session_cb)(struct ssl_st *ssl3 , unsigned char *data3 , int len3 ,
                                  int *copy3 ) ;
   ...
   unsigned int sid_ctx_length ;
   unsigned char sid_ctx[32] ;
   int (*default_verify_callback)(int ok4 , X509_STORE_CTX *ctx4 ) ;
   int purpose ;
   int trust ;
...
struct cert_st {
   ...
   RSA *rsa_tmp ;
   RSA *(*rsa_tmp_cb)(SSL *ssl1 , int is_export1 , int keysize1 ) ;
   DH *dh_tmp ;
   DH *(*dh_tmp_cb)(SSL *ssl2 , int is_export2 , int keysize2 ) ;
   CERT_PKEY pkeys[5] ;
   int references ;
};
...
extern void ERR_put_error(int lib , int func , int reason , char  *file , int line ) ;
...
extern int ssl3_put_cipher_by_char(SSL_CIPHER  *c , unsigned char *p ) ;
...
extern int ssl3_send_finished(SSL *s , int a , int b , char  *sender , int slen ) ;
...
int main()
{
...

  {
	...
	// Should we shadow also (s->s3)->tmp.next_state?
	break;
      case 8576: ;
      case 8577:
	...
	else _ABORT(blastFlag == 5); // if(blastFlag == 5) goto ERROR; // 
	if (ret <= 0) {
	  goto end;
	}
	s->state = 8448;
	...
	if (got_new_session) {
	  ...
	  // TRACER  s->handshake_func = (int (*)())(& ssl3_accept);
	  if ((unsigned long )cb != (unsigned long )((void *)0)) { 
	    //((*cb))(s, 32, 1);
	  }
	} ret = 1;
