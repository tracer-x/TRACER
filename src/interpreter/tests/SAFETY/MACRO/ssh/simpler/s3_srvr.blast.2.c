...
{ 
...

  {
    ...
    //tmp___1 = SSL_state(s); 
    //tmp___1 = unknown();
    if (TRACER_NONDET /* & 12288 */) { 
      //tmp___2 = SSL_state(s); 
      //tmp___2 = unknown();
      if (TRACER_NONDET /* & 16384 */) { 
	//SSL_clear(s);
      }
    } else {
      //SSL_clear(s);
    } 
    if (unknown() /*(unsigned long )s->cert*/ == (unsigned long )((void *)0)) {
	...
	s->new_session = 1;
      ...
      case 8195:
	s->server = 1; 
	if (unknown() /* (unsigned long )cb */ != (unsigned long )((void *)0)) { 
	  //((*cb))(s, 16, 1);
	} 
	if (unknown() /* s->version >> 8 */ != 3) {
	...
	else if(blastFlag == 3) blastFlag = 4;
	...
	break;
      case 8512: ;
      case 8513: ;
	if (unknown() /*((s->s3)->tmp.new_cipher)->algorithms & 256UL*/) {
	  skip = 1;
	} else {
...
/* 	} */
/* 	if (unknown() /\*(s->s3)->tmp.use_rsa_tmp*\/) { */
/* 	  goto _L___0; */
/* 	} else { */
/* 	  if (l & 30UL) { */
/* 	    goto _L___0; */
/* 	  } else { */
/* 	    if (l & 1UL) {  */
/* 	      if (unknown() /\*(unsigned long )(s->cert)->pkeys[0].privatekey == (unsigned long )((void *)0)*\/) { */
/* 		goto _L___0; */
/* 	      } else { */
/* 		if (unknown() /\*((s->s3)->tmp.new_cipher)->algo_strength & 2UL*\/) {  */
/* 		  //tmp___6 = EVP_PKEY_size((s->cert)->pkeys[0].privatekey);  */
/* 		  tmp___6 = unknown(); */
/* 		  if (unknown() /\*((s->s3)->tmp.new_cipher)->algo_strength & 4UL*\/) {  */
/* 		    tmp___7 = 512; */
/* 		  } else { */
/* 		    tmp___7 = 1024; */
/* 		  } */
/* 		  if (unknown() /\*tmp___6 * 8*\/ > tmp___7) { */
/* 		  _L___0: */
/* 		  _L: */
...
/* 	} */
	...
	break;
      case 8544: ;
      case 8545: ;
	if (unknown() /*s->verify_mode & 1*/) { 
	  if (unknown() /*(unsigned long )(s->session)->peer*/ != (unsigned long )((void *)0)) { 
	    if (unknown() /*s->verify_mode & 4*/) {
	      skip = 1;
	...
	break;
      case 8672: ;
      case 8673:
	//ret = ssl3_send_finished(s, 8672, 8673, ((s->method)->ssl3_enc)->server_finished_label,((s->method)->ssl3_enc)->server_finished_lab_len);
	ret = unknown();
	_TRACER_abort(blastFlag == 4); 
	if (ret <= 0) {
	  goto end;
	}
	myState /*s->state*/ = 8448;
...
/* 	}  */
	...
	goto end;
      ...
      if (! unknown() /*(s->s3)->tmp.reuse_message*/) { 
	if (/*! skip*/ TRACER_NONDET) {
	  if (s->debug) {
