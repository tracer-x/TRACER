...
{ 
...

  {
	...
	s->new_session = 1;
      ...
      case 8195:
	s->server = 1;
	if (/*(unsigned long )cb != (unsigned long )((void *)0)*/ TRACER_NONDET) { 
	  //((*cb))(s, 16, 1);
	} 
	if (s->version >> 8 != 3) {
	...
	else if(blastFlag == 15) blastFlag = 16;
	else if(blastFlag == 18) blastFlag = 19;
	else _TRACER_abort(blastFlag == 21); //if(blastFlag == 21) goto ERROR;
	if (ret <= 0) {
	  goto end;
	}
	...
	else if(blastFlag == 16) blastFlag = 17;
	else if(blastFlag == 19) blastFlag = 20;
	if (ret <= 0) {
	  goto end;
	}
	...
	else if(blastFlag == 17) blastFlag = 18;
	else if(blastFlag == 20) blastFlag = 21;
	if (ret <= 0) {
	  goto end;
	}
	myState /*s->state*/ = 8448;
	...
	s->init_num = 0;
	if (/*got_new_session*/ TRACER_NONDET) {
	  s->new_session = 0;
