...
{ 
...

  {
	...
	(s->ctx)->stats.sess_connect_renegotiate ++;
      ...
      case 4099:
	s->server = 0;
	if (TRACER_NONDET) { //((unsigned long )cb != (unsigned long )((void *)0)) { 
	  //((*cb))(s, 16, 1);
	} 
	...
	if (unknown() /*(s->s3)->tmp.cert_req*/ == 1) {
	...
	if (num1 > 0L) {
	  ...
	  //tmp___9 = unknown();
	  num1 = TRACER_NONDET; //(long )((int )tmp___9);
	  if (num1 <= 0L) { 
	    ret = -1;
	    goto end;
	  }
...
{ STACK *sk ;
  ...
  if (unknown() /*s->hit*/) { if ((unsigned long )(s->session)->cipher != (unsigned long )c) {
      if (! (s->options & 8UL)) {
        al = 47;
...
{ X509 *x509 ;
  ...
  if (s->state == 4466) {
    s->state = 4467;
    if (unknown() /*(s->s3)->tmp.cert_req*/ == 2) { tmp___1 = (X509 *)((void *)0);
    } else {
