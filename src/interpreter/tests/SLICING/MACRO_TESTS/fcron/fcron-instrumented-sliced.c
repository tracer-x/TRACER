...
{ 
  ...
  num_strings = 0;
  // # 24
  if ((unsigned int )array_in == (unsigned int )((char * /*__attribute__((__safe__))*/  *)0)) {
    return ((char **)0);
  }
// # 28
  p = array_in;
...
{ 
  ...
  parseopt(argc, argv);
  read_conf();
  tmp___1 = chdir((char *)fcrontabs);
  if (tmp___1 != 0) {
    die_e("Could not change dir to %s", fcrontabs);
  }
  tmp___2 = (FILE *)get_stdin();
  tmp___2->BLAST_FLAG = 1;
  freopen((char    *)((char  *)"/dev/null"),
          (char    *)((char  *)"r"), tmp___2);
  ...
  if ((int )foreground == 0) {
    ...
    tmp___3 = (FILE *)get_stdout();
    tmp___3->BLAST_FLAG = 1;
    freopen((char    *)((char   *)"/dev/null"),
            (char    *)((char   *)"w"), tmp___3);
    tmp___4 = (FILE *)get_stderr();
  ...
  //slice_criteria = BLAST_error; /*For CodeSurfer*/
  return (0); 
  _SLICE(BLAST_error);
}
...
{ 
  ...
  if ((int )sig_conf > 0) {
    if ((int )sig_conf == 1) {
      // TRACER short synchronize_dir((char /*const*/   *)".");
      sig_conf = (char )0;
      signal(1, & sighup_handler);
    } else {
      reload_all((char  *)".");
...
{ 
  ...
  while (1) {
    if (stime___0 > 1L) {
      tv.tv_sec = stime___0 - 1;
    } else {
      tv.tv_sec = 0;
    }
    retcode = select(set_max_fd + 1, (fd_set *)(& read_set),
                     (fd_set *)((fd_set *)((void *)0)),
                     (fd_set *)((fd_set *)((void *)0)),
                     (struct timeval *)(& tv));
    if (retcode < 0) {
      tmp___0 = __errno_location();
    ...
    if (save <= now) {
      save = now + save_time;
      /*save_file((struct cf_t *)((void *)0));*/ // heap overflow
    }

    /*check_socket(retcode);*/         // heap overflow
    /*stime___0 = check_lavg(save);*/  // heap overflow
    if (debug_opt) {
...
// # 205
void explain_fd(int fd , char *fmt  , ...) {  return; }

// # 217
void explain_e(char *fmt  , ...)  { return; }
...
{ 
  ...
  _L: 
    ...
    if ((int )line->cl_numexe >= 255) {
      ...
      goto _L; // it's fine it's not a loop
    }
  }
...
{ 
  ...
  _L___0: 
	...
	// # 369
      ...
      goto _L___0; // it's fine it's not a loop
    }
  }
  // # 376
  lavg_num = (short )((int )lavg_num + 1);
...
{ 
  ...
  // # 68
  // TRACER short synchronize_dir(dir_name);
  // # 70
  return;
}
