...
// #line 93
void * memcpy(void *  p1, const void * s1 , size_t t1){
  void * x; return x;
}
void * memset(void *  p2, int  s2, size_t t2){
  void * x; return x;
}
...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject , PUNICODE_STRING RegistryPath)
{
  ...
  DiskPerfRegistryPath.Buffer = ExAllocatePoolWithTag(PagedPool,
						      DiskPerfRegistryPath.MaximumLength, tag__1 /*'frPD'*/);
  // #line 432
				  ...
				  PDEVICE_OBJECT TargetDevice )
{
  ...
  FilterDevice->Flags = FilterDevice->Flags | propFlags;
  propFlags = TargetDevice->Characteristics & (0x00000001 | 0x00000002 | 0x00000004);
  FilterDevice->Characteristics = FilterDevice->Characteristics | propFlags;
}
			   ...
			   PDEVICE_OBJECT PhysicalDeviceObject )
{
  ...
  filterDeviceObject->Flags = filterDeviceObject->Flags | 0x00000010;
  ...
  filterDeviceObject->Flags = filterDeviceObject->Flags | 0x00002000;
  // filterDeviceObject->Flags = filterDeviceObject->Flags & (~ 0x00000080);
  return (NTSTATUS )0x00000000L;
}
...
NTSTATUS DiskPerfDispatchPnp(PDEVICE_OBJECT DeviceObject , PIRP Irp )
{
  ...
  switch(irpSp->MinorFunction)
	...
	//  *
	if(! InPath && deviceExtension->PagingPathCount == 1)
...
{
  DeviceObject;
  (Irp->IoStatus).Status = (NTSTATUS )0 /* 0x00000000L */;
	  ...
	  if(BufferAvail >= sizeNeeded)
	      ...
	      if(totalCounters->QueueDepth == 0)
		{
		  ...
		  if(frequency.QuadPart > 0)
		    {
		      (totalCounters->IdleTime).QuadPart += (10000000 * difference.QuadPart /	      
							     frequency.QuadPart);
		    }
		}
	      totalCounters->StorageDeviceNumber = deviceExtension->DiskNumber;		
