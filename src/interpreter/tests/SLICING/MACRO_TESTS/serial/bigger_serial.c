...
NTSTATUS SerialClose(PDEVICE_OBJECT DeviceObject , PIRP Irp )
{
  ...
  for(flushCount = BLAST_NONDET; flushCount != 0; flushCount--)
    {
      temp_1=(READ_PORT_UCHAR(extension->Controller + (ULONG )(0x05 * 1)) & (0x20 | 0x40)); 
      if(temp_1 != (0x20 | 0x40))
	{
...
BOOLEAN SerialCheckOpen(PVOID Context )
{
  ...
  // TRACER: some problem with compilation don't know why ???!!!??
/*   do */
/*     { */
/*       currentExtension = (SERIAL_DEVICE_EXTENSION *  )((PCHAR)currentEntry - */
      ...
      else
	{
	  ...
	  * & controlIrql = KfAcquireSpinLock(& Extension->ControlLock);
	  ...
	  if(returnWithWhatsPresent || !
	    ...
	    {
	      ((((((Extension->CurrentReadIrp)->Tail).Overlay).CurrentStackLocation)->Parameters).Others).Argument4 = (void * )0;
...
ULONG SerialGetCharsFromIntBuffer(PSERIAL_DEVICE_EXTENSION Extension )
{
  ...
  if(numberOfCharsToGet)
	...
	{
	  memmove((PUCHAR)((Extension->CurrentReadIrp)->AssociatedIrp).SystemBuffer +
		  (((((((Extension->CurrentReadIrp)->Tail).Overlay)
		      .CurrentStackLocation)->Parameters).Read).Length -
      ...
      else
	{
	  ...
	  Extension->FirstReadableChar =
	    Extension->InterruptReadBuffer + (numberOfCharsToGet - firstTryNumberToGet);
	  Extension->NumberNeededForRead -= numberOfCharsToGet -
	    firstTryNumberToGet;
	}
    }
