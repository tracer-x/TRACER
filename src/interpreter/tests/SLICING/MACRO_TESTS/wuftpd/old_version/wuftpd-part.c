...
off_t byte_count  ;
int TCPwindowsize  ;
int retrieve_is_data  ;
char LastFileTransferred[4096]  ;
static char *RootDirectory  ;
unsigned int defumask  ;
char tmpline[7]  ;
char hostname[64]  ;
char remotehost[64]  ;
char remoteaddr[64]  ;
...
int keepalive ;
int xferlog  ;
int log_outbound_xfers  ;
int log_incoming_xfers  ;
char logfile[4096]  ;
int dolreplies  ;
char *autospout  ;
int autospout_free  ;
int mangleopts  ;
int lgi_failure_threshold  ;
int swaitmax  ;
int swaitint  ;
char proctitle[8192]  ;
char ls_long[1024]  ;
...
// ----------------------------------
extern void syslog(void a1,void a2, void a3);  // Added by Jorge
extern int inet_ntoa(void cliaddr);            // Added by Jorge 
...

  {
	...
	if (cliaddr.s_addr == his_addr.sin_addr.s_addr) {
	  ...
	  if (tmp___3) {
	    ...
	    syslog(4, "refused PORT %s,%d from %s", tmp___1, __v, remoteident);
...
/*  points-to-set([((int)__x___0), __x___0]) */
