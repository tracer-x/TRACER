/* To compile: gcc -lm -o susan_thin susan_thin.c */
  ...
  FILE *fd;
{
  ...
  c = getc(fd);
  //Commented by Jorge:  while (1) /* find next integer */
  for (k=0;k<5;k++)
  {
  ...
  i = 0;
  for (k=0;k<5;k++) //Commented by Jorge while (1) {    
    i = (i*10) + (c - '0');
    c = getc(fd);
    if (c==EOF) return (i);
    if (c<'0' || c>'9') break;
...
int   i, j;
  ...
  for(i=0; i<*y_size; i++)  // ITER? 5  /* copy *in into tmp_image */  
    memcpy(tmp_image+(i+border)*(*x_size+2*border)+border, *in+i* *x_size, *x_size);

  for(i=0; i<border; i++)   // ITER? 5 /* copy top and bottom rows; invert as many as necessary */
  {
  ...
  for(i=0; i<border; i++)                 // ITER? 5 /* copy left and right columns */
...
/* }}} */
        ...
        if ( (n==1) && (mid[i*x_size+j]<6) )
        {
          ...
          for(y=0; y<3; y++)       // ITER 2
            for(x=0; x<3; x++)     // ITER 2
