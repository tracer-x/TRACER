...
NTSTATUS do_while_abstraction_1(){
...
NTSTATUS do_while_abstraction_2(){
 ...
 PioneerRestart:
  ...
  switch(NONDET/* ((currentIrpStack->Parameters).DeviceIoControl).IoControlCode */)
	...
	status = SendSrbSynchronous(deviceExtension, & srb, (void *  )0, 0);
      ...
      // TRACER  while(! ((NTSTATUS )status >= 0) && retry-- > 0);
      status =  do_while_abstraction_2();
	...
	status = SendSrbSynchronous(deviceExtension, & srb, SubQPtr, 6);
      ...
      // TRACER while(! ((NTSTATUS )status >= 0) && retry-- > 0 && status != (NTSTATUS )0xC00000A3L);	 
      status = do_while_abstraction_1();
