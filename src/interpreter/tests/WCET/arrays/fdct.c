...
void fdct(int lx)
{
  ...
  for (i=0; i<8; i++)
  {
    ...
    block[offset+0] = ((tmp10+tmp11) * PASS1_BITS);
    block[offset+4] = ((tmp10-tmp11) * PASS1_BITS);

    constant= 4433;
    z1 = (tmp12 + tmp13) * constant;
    constant= 6270;
    block[offset+2] = (z1 + (tmp13 * constant)) / (CONST_BITS-PASS1_BITS);
    constant= -15137;
    block[offset+6] = (z1 + (tmp12 * constant)) / (CONST_BITS-PASS1_BITS);
    ...
    block[offset+7] = (tmp4 + z1 + z3) / (CONST_BITS-PASS1_BITS);
    block[offset+5] = (tmp5 + z2 + z4) / (CONST_BITS-PASS1_BITS);
    block[offset+3] = (tmp6 + z2 + z3) / (CONST_BITS-PASS1_BITS);
    block[offset+1] = (tmp7 + z1 + z4) / (CONST_BITS-PASS1_BITS);
  ...
  for (i = 0; i<8; i++)
    ...
    block[i+0] = (tmp10 + tmp11) / (PASS1_BITS+3);
    block[i+4*lx] = (tmp10 - tmp11) / (PASS1_BITS+3);

    constant = 4433;
    z1 = ((tmp12 + tmp13) * constant);
    constant= 6270;
    block[i+2*lx] = (z1 + (tmp13 * constant)) / (CONST_BITS+PASS1_BITS+3);
    constant=-15137;
    block[i+6*lx] = (z1 + (tmp12 * constant)) / (CONST_BITS+PASS1_BITS+3);
    ...
    block[i+7*lx] = (tmp4 + z1 + z3) / (CONST_BITS+PASS1_BITS+3);
    block[i+5*lx] = (tmp5 + z2 + z4) / (CONST_BITS+PASS1_BITS+3);
    block[i+3*lx] = (tmp6 + z2 + z3) / (CONST_BITS+PASS1_BITS+3);
    block[i+lx] =  (tmp7 + z1 + z4) / (CONST_BITS+PASS1_BITS+3);
