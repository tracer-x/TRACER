...
static void TLan_SetTimer( register struct net_device *dev, register u32 ticks, register u32 type )
{
...
static void tlan_remove_one( register struct pci_dev *pdev)
{
...
static int tlan_probe(void)
{
  ...
  register int temp_1,temp_2,temp_3,temp_4;
...
int TLan_probe1(register struct pci_dev *pdev,register long ioaddr, register int irq, register int rev, 
		register struct pci_device_id *ent )		       
{
...
static void TLan_EisaProbe (void)
{
  ...
  register unsigned char temp_2,temp_3, temp_4;
...
static int TLan_Init( register struct net_device *dev )
{
  register int dma_size;
  register int err;
  register int i;
  register TLanPrivateInfo *priv;
  register int temp_1,temp_2,temp_3;
...
static int TLan_Open( register struct net_device *dev )
{
  register TLanPrivateInfo *priv;
  register int err;
...
static int TLan_ioctl(register struct net_device *dev, register struct ifreq *rq, register int cmd)
{
  register TLanPrivateInfo *priv;
...
static int TLan_StartTx( register struct sk_buff *skb, register struct net_device *dev )
{
  register TLanPrivateInfo *priv;
...
static void TLan_HandleInterrupt(register int irq, register void *dev_id, register struct pt_regs *regs)
{
  register u32 ack;
  register struct net_device *dev;
...
static void TLan_SetMulticastList( register struct net_device *dev )
{
...
u32 TLan_HandleTxEOF( register struct net_device *dev, register u16 host_int )
{
  register TLanPrivateInfo *priv;
  register int eoc;
  ...
  register int temp_1, temp_2, temp_4, temp_5;
...
u32 TLan_HandleStatOverflow( register struct net_device *dev, register u16 host_int )
{
  TLan_ReadAndClearStats( dev, 1 );  
  return 1;
...
u32 TLan_HandleRxEOF( register struct net_device *dev, register u16 host_int )
{
  register TLanPrivateInfo *priv;
  register u32 ack;
  register int eoc;
...
u32 TLan_HandleStatusCheck( register struct net_device *dev, register u16 host_int )
{
  register TLanPrivateInfo *priv;
  register u32 ack;
  register u32 error;
...
void TLan_Timer( register unsigned long data )
{
  register struct net_device *dev;
  register TLanPrivateInfo *priv;
...
void TLan_ResetLists( register struct net_device *dev )
{
  register TLanPrivateInfo *priv;
  register int i;
...
void TLan_FreeLists( register struct net_device *dev )
{
  register TLanPrivateInfo *priv = dev->priv;
  register int i;
  register TLanList *list;
  register struct sk_buff *skb;
...
void TLan_ReadAndClearStats( register struct net_device *dev, register int record )
{
  register TLanPrivateInfo *priv;
  register u32 tx_good, tx_under;
  register u32 rx_good, rx_over;
  register u32 def_tx, crc, code;
  register u32 multi_col, single_col;
  register u32 excess_col, late_col, loss;
...
void TLan_ResetAdapter( register struct net_device *dev )
{
  register TLanPrivateInfo *priv;
  register int i;
  register u32 addr;
...
void TLan_FinishReset( register struct net_device *dev )
{
  register TLanPrivateInfo *priv = dev->priv;
  register u8 data;
  register u32 phy;
...
void TLan_SetMac(register  struct net_device *dev, register int areg, register char *mac )
{
  register int i;
...
void TLan_PhyPrint(register  struct net_device *dev )
{
  register TLanPrivateInfo *priv;
...
void TLan_PhyDetect( register struct net_device *dev )
{
  register TLanPrivateInfo *priv = dev->priv;
  u16 control;
  u16 hi;
...
void TLan_PhyReset( register struct net_device *dev )
{
  register TLanPrivateInfo *priv;
  register u16 phy;
  u16 value;
...
void TLan_PhyStartLink( register struct net_device *dev )
{
  register TLanPrivateInfo *priv;
  u16 ability;
...
void TLan_PhyFinishAutoNeg( register struct net_device *dev )
{
  register TLanPrivateInfo *priv;
  u16 an_adv;
  u16 an_lpa;
...
int TLan_MiiReadReg( register struct net_device *dev, register u16 phy, register u16 reg, register u16 *val )
{
  register u8 nack;
  register u16 sio, tmp;
  register u32 i;
  register int err;
  register int minten;
  register TLanPrivateInfo *priv;
...
void TLan_MiiSendData( register u16 base_port, register u32 data, register unsigned num_bits )
{
  register u16 sio;
  register u32 i;
...
void TLan_MiiWriteReg(register  struct net_device *dev, register u16 phy, register u16 reg, register u16 val )
{
  register u16 sio;
  register int minten;
  register unsigned long flags;
  ...
  register unsigned char temp1,temp2;
...
int TLan_EeSendByte(register  u16 io_base, register u8 data, register int stop )
{
  register int err;
  register u8 place;
  register u16 sio;
...
void TLan_EeReceiveByte( register u16 io_base, register u8 *data, register int stop )
{
  register u8 place;
  register u16 sio;
  register int temp_1;
...
int TLan_EeReadByte( register struct net_device *dev, register u8 ee_addr, register u8 *data )
{
  register int err;
  register TLanPrivateInfo *priv;
  register unsigned long flags;
