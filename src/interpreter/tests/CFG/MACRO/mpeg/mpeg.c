...
extern int previous_picture/*  =0 */;
extern int future_picture/*  = 24 */;
...
void init()
{
 /* Vijay: init() is called only once, and is used to initialize global variables.
  * doesn't matter even if it's commented for decompilation, as it doesn't
...
/* End commented by Jorge */
 ...
 for (i=0;i<5;i++) // ITER 5
 {
  ...
  //  {
   ...
   {
     while ((to_receive!=0)  && (iobuf_look!=iobuf_hd_in))  // ITER? 5
    {
...
void next_vld(register int start,register int max_next_vld)
{ 
 register int i;
...
/* Vijay - these variables were global, but they were only used
 * in vldcontrol() (which is called only once), so making them local
 * so that they can be declared register vars */
...
void idct()
{
 ...
 for (i=0; i<=63; i++)  // ITER 64
  temp_block[i]=0;
 
 // // Temporary_block = idct_coeff * curr_block //
 for (i=0; i<=7; i++)  // ITER 8
 {
  for (j=0; j<=7; j++) // ITER 8
  {
   for (k=0; k<=7; k++) // ITER 8
...
void calculate_forward_motion(register int new_prev_values)
{
...
void calculate_backward_motion(register int new_prev_values)
{
...
void calcid(register int picture, register int mb_addr, register int i, register int down_for_calcid, register int j, register int right_for_calcid)
{
