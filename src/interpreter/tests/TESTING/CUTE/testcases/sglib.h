...
\
static int sglib___##type##_consistency_check_recursive(type *t, int *pathdeep, int cdeep) {\
  if (t==NULL) {\
    if (*pathdeep < 0) *pathdeep = cdeep;\
    else if(!(*pathdeep == cdeep)) return 0;	 \
  } else {\
    if (t->left!=NULL) if(!(comparator(t->left, t) <= 0)) return 0;	\
    if (t->right!=NULL) if(!(comparator(t, t->right) <= 0)) return 0;		\
    if (SGLIB___GET_VALUE(t->bits) == RED) {\
      if(!(t->left == NULL || SGLIB___GET_VALUE(t->left->bits)==BLACK)) return 0; \
      if(!(t->right == NULL || SGLIB___GET_VALUE(t->right->bits)==BLACK)) return 0; \
      if(!sglib___##type##_consistency_check_recursive(t->left, pathdeep, cdeep)) return 0; \
      if(!sglib___##type##_consistency_check_recursive(t->right, pathdeep, cdeep)) return 0; \
    } else {\
      if(!sglib___##type##_consistency_check_recursive(t->left, pathdeep, cdeep+1)) return 0; \
      if(!sglib___##type##_consistency_check_recursive(t->right, pathdeep, cdeep+1)) return 0; \
    }\
  }\
  return 1;\
}\
\
int sglib___##type##_consistency_check(type *t) {\
  int pathDeep;\
  if(!(t==NULL || SGLIB___GET_VALUE(t->bits) == BLACK))  return 0;	\
  pathDeep = -1;\
  return sglib___##type##_consistency_check_recursive(t, &pathDeep, 0);\
}
...
#define SGLIB_DEFINE_RBTREE_PROTOTYPES(type, left, right, colorbit, comparator) \
 ...
 extern int sglib___##type##_consistency_check(type *t); \
